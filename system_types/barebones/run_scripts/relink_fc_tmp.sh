...
for expt in Thom_GTS+ch08_short30min; do
  ...
  cd $WORK_DIR/fc_noAMV
  for date in `ls`; do
    echo $date
    cd $WORK_DIR/fc_noAMV/$date

    # Determinin the previous date
    if [[ $date == 201705310600 ]]; then
      prev_date=201705310530
    else
      prev_date=`advance_time $date -$tint`
    fi

    # Ignore spinup
    if [[ $date == 201705310530 ]]; then
      continue
    fi
    ...
    for mem in `seq 1 $NUM_ENS`; do
      ...
      # If not NoDA, we need to link a load of stuff
        ...
        nn=$((90010+$mem))
        ln -sf $WORK_DIR/run_noAMV/$date/enkf/d01/fort.$nn wrf_enkf_output_d01_$eee
