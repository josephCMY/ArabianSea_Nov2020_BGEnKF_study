...
if [[ $domain_id < 3 ]]; then USE_RADAR_RV=false; USE_AIRBORNE_RV=false; fi
...
&hurricane_PI 
use_hurricane_PI  = .$USE_hurricane_PI.,
hroi_hurricane_PI = $((HROI_HPI/$dx)),
vroi_hurricane_PI = 43,
/
...
&coplane_radar
use_coplane_uv    = .$USE_COPLANE_UV.,
datathin_coplane  = $THIN_RADAR,
hroi_coplane      = $((HROI_RADAR/$dx)),
vroi_coplane      = $VROI_RADAR,
/
