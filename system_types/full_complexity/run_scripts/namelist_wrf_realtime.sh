...
#  echo start_day          = `for i in $domlist2; do printf ${start_date:6:2}, ; done`${DOMAIN4_START:6:2},
#  echo start_hour         = `for i in $domlist2; do printf ${start_date:8:2}, ; done`${DOMAIN4_START:8:2},
...
if $RESTARTING; then
  echo restart                             = .true.,
  echo restart_interval                              = 1440,
#elif ! $FIRST_CYCLE; then
#  echo restart                             = .true.,
#  echo restart_interval                    = `printf $run_minutes,`
else
  echo restart                             = .false.,
  #echo restart_interval                    = `printf $run_minutes,`
  echo restart_interval                    = 7200,
fi
cat << EOF
debug_level        = 1500,
EOF
...
if [[ $use_for == "ndown" ]]; then
  ...
  echo inputout_interval=$inputout_interval,
  echo inputout_begin_m=$inputout_begin,
  echo inputout_end_m=$inputout_end,
fi
...
EOF
elif [[ $use_for == "ndown" ]] && [[ $idom == 4 ]]; then
cat << EOF
grid_id    = 3,4,
parent_id  = 2,3,
parent_grid_ratio = ${GRID_RATIO[$idom-2]},${GRID_RATIO[$idom-1]},
...
if  ! $MOVING_NEST && [[ $use_for != "perturb" ]] && [[ $use_for != "ndown" ]] && [[ $wrf_for != "forecast" ]]; then
  cat domain_moves
elif $FIRST_CYCLE; then
  cat domain_moves
elif ! $FIRST_CYCLE && [[ $wrf_for == "forecast" ]]; then 
  echo time_to_move = $TIME_TO_MOVE.,
