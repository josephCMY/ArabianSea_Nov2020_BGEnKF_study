...
if $REPLACE_ENVIRONMENT; then
 ...
 if [ $DATE == $LBDATE ]; then
  tid=0
  if [[ ! -d $WORK_DIR/fc_no_gfs ]]; then mkdir $WORK_DIR/fc_no_gfs; fi  # Added 7.1.20 CMH
  mkdir $WORK_DIR/fc_no_gfs/$DATE     # Added 7.1.20 CMH
  for n in $domlist; do
#  for n in `seq 1 $((MAX_DOM-1))`; do
    ...
    for NE in `seq 1 $((NUM_ENS+1))`; do
      ...
      ln -sf $TCVITALS_DIR/${DATE:0:4}/${DATE}.${STORM_ID}-tcvitals.dat tcvitals.dat
      cp ../../fort.`expr 80010 + $NE` ../../fort.`expr 80010 + $NE`_no_gfs  # Added 7.1.20 CMH
      cp ../../fort.`expr 90010 + $NE` ../../fort.`expr 90010 + $NE`_no_gfs  # Added 7.1.20 CMH
...
for n in $domlist; do
  ...
  for NE in `seq 1 $NUM_ENS`; do
    ...
    # Added 7.1.20 by CMH
    if [[ -d $WORK_DIR/fc_no_gfs/$DATE ]]
    then
      mv $dm/fort.`expr 80010 + $NE`_no_gfs $WORK_DIR/fc_no_gfs/$DATE/wrf_enkf_input_${dm}_$id
      mv $dm/fort.`expr 90010 + $NE`_no_gfs $WORK_DIR/fc_no_gfs/$DATE/wrf_enkf_output_${dm}_$id
      ln -fs $WORK_DIR/fc_no_gfs/$DATE/wrf_enkf_output_${dm}_$id $WORK_DIR/fc_no_gfs/$DATE/wrfinput_${dm}_$id
    fi    
  done
  ...
  if [[ -d $WORK_DIR/fc_no_gfs/$DATE ]]
  then
     cp $dm/fort.`expr 80011 + $NUM_ENS`_no_gfs $WORK_DIR/fc_no_gfs/$DATE/wrf_enkf_input_${dm}_mean
     cp $dm/fort.`expr 90011 + $NUM_ENS`_no_gfs $WORK_DIR/fc_no_gfs/$DATE/wrf_enkf_output_${dm}_mean
     ln -fs $WORK_DIR/fc_no_gfs/$DATE/wrf_enkf_output_${dm}_mean $WORK_DIR/fc_no_gfs/$DATE/wrfinput_${dm}
  fi    
done
