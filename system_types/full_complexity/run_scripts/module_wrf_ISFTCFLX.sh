#!/bin/bash
. $CONFIG_FILE
rundir=$WORK_DIR/run/$DATE/wrf_ISFTCFLX$ISFTCFLX_FCST
if [[ ! -d $rundir ]]; then mkdir -p $rundir; echo waiting > $rundir/stat; fi
if [[ ! -d $WORK_DIR/output/$DATE/ISFTCFLX$ISFTCFLX_FCST ]]; then mkdir -p $WORK_DIR/output/$DATE/ISFTCFLX$ISFTCFLX_FCST/; fi
cd $rundir
...
#Setup for wrf run
echo running > stat
echo "  Running WRF forecast ISFTCFLX$ISFTCFLX_FCST..."
...
for i in 1; do
  ...
  #$SCRIPT_DIR/namelist_wrf_realtime.sh wrfw > namelist.input
  ...
  #else
  $SCRIPT_DIR/job_submit.sh $wrf_ntasks_fcst 0 $HOSTPPN ./wrf.exe >& wrf.log &
  #fi
wait
done
...
#wrfout files save to output dir
  for n in `seq 1 $MAX_DOM_FCST`; do
    dm=d`expr $n + 100 |cut -c2-`
    outdate=$DATE
     cp namelist.input $WORK_DIR/output/$DATE/ISFTCFLX$ISFTCFLX_FCST/.
    while [[ $outdate -le `advance_time $DATE $run_minutes` ]]; do
      watch_file wrfout_${dm}_`wrf_time_string $outdate` 1 $rundir
      mv wrfout_${dm}_`wrf_time_string $outdate` $WORK_DIR/output/$DATE/ISFTCFLX$ISFTCFLX_FCST/.
