#!/bin/bash
. $CONFIG_FILE
rundir=$WORK_DIR/run/$DATE/wrf_3km
if [[ ! -d $rundir ]]; then mkdir -p $rundir; echo waiting > $rundir/stat; fi
if [[ ! -d $WORK_DIR/output/$DATE/wrf_3km ]]; then mkdir -p $WORK_DIR/output/$DATE/wrf_3km/; fi
...
for i in 1; do
  ...
  $SCRIPT_DIR/namelist_wrf_ISFTCFLX.sh wrfw > namelist.input
...
#wrfout files save to output dir
  for n in `seq 1 $MAX_DOM`; do
    dm=d`expr $n + 100 |cut -c2-`
    outdate=$DATE
     cp namelist.input $WORK_DIR/output/$DATE/wrf_3km/.
    while [[ $outdate -le `advance_time $DATE $run_minutes` ]]; do
      watch_file wrfout_${dm}_`wrf_time_string $outdate` 1 $rundir
      mv wrfout_${dm}_`wrf_time_string $outdate` $WORK_DIR/output/$DATE/wrf_3km/.
