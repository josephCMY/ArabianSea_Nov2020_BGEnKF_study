...
#    print( "\n\n\nDealing with %s" % (raw_flist[ff]))
    ...
    for ll in range( 21, nL ):
        if raw_lines[ll].find( 'FM-12') != -1 or  raw_lines[ll].find( 'FM-16') != -1:
            obsflags[ll] = True
...
sc = bm.scatter( xx,yy, s=20, c=colors, label = 'sounding_sites',  zorder=1000, edgecolor='white', linewidth=0.5, cmap = 'jet', vmin=1., vmax=12)
plt.title('METAR and SYNOP Sites')
fig.subplots_adjust( left=0.03)
cbar_ax = fig.add_axes([0.85, 0.15, 0.05, 0.7])
fig.colorbar(sc, cax=cbar_ax)


plt.savefig('surface_sites.png', dpi=200 )
plt.close()
