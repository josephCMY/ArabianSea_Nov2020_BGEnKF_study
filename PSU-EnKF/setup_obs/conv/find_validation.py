''' 
Script to plot out locations of soundings suitable for validation.
Properties of validation soundings:
    1) Need to have at least one sounding value per 100 hPa.
    2) Need to be at least 2x/day.
...
#    print( "\n\n\nDealing with %s" % (raw_flist[ff]))
    ...
    for oo in np.arange(nObs):
        ...
        # Now we want at least one temperature obs per 200 hPa
        if np.sum(cntP) > 0:
            cnt2, xe, ye = np.histogram2d( gts_obs[oo]['vert']['pres']['data'], gts_obs[oo]['vert']['temp']['data'],
                                           bins = ( np.linspace( 100*100, 900*100, 5), np.linspace( 100,350, 3)) )
        else:
            cnt2, xe, ye = np.histogram2d( gts_obs[oo]['vert']['hgt']['data'], gts_obs[oo]['vert']['temp']['data'],
                                           bins = ( zbins[::2], np.linspace( 100,350, 3)) )
        
        # Check temperature obs density
        cnt2 =  np.sum(cnt2, axis=1)
        flags = (cnt2==0)
        if np.sum(flags):
            continue
        
        # Now we want at least one dewpt obs per 200 hPa
        if np.sum(cntP) > 0:
            cnt2, xe, ye = np.histogram2d( gts_obs[oo]['vert']['pres']['data'], gts_obs[oo]['vert']['dwpt']['data'],
                                           bins = ( np.linspace( 100*100, 900*100, 5), np.linspace( 0,350, 3)) )
        else:
            cnt2, xe, ye = np.histogram2d( gts_obs[oo]['vert']['hgt']['data'], gts_obs[oo]['vert']['dwpt']['data'],
                                           bins = ( zbins[::2], np.linspace( 0,350, 3)) )
...
for utc in loc_contain.keys():
    ...
    if loc_contain[utc]['latlon'].shape[0] > 0:
        ...
        if utc != 'all':
            ...
            hist = np.ma.masked_where(hist < 2., hist)
            bm.pcolormesh( xx,yy, hist.T, latlon=True, vmin=2., vmax=2.01, cmap=plt.cm.bwr)
            # Print out locations with at least 2 obs
            site_list = []
            for ii in range( len(xc)):
                for jj in range(len(yc)):
                    if hist[ii,jj] >= 2.:
                        site_list.append( [xc[ii],yc[jj]] )

        cb = plt.colorbar()
        cb.ax.set_ylabel('Obs / day')
        plt.savefig('vert_obs_spatial_distribution_%s.png' % utc, dpi=200 )
    plt.close()
