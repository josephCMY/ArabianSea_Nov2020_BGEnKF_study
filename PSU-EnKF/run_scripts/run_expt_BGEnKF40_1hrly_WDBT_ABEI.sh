...
#SBATCH -o log.expt_BGEnKF40_1hrly_WDBT_ABEI
#SBATCH -e err.expt_BGEnKF40_1hrly_WDBT_ABEI

export SLURM_NTASKS=680
...
export CONFIG_FILE=~/nonlinear_IR-DA/indian_ocean_osse/PSU_EnKF/scripts/config/BGEnKF40_1hrly_WDBT_ABEI
. $CONFIG_FILE
cd $SCRIPT_DIR
. util.sh
...
if [[ ! -d run/$DATE_START ]]; then
  ...
  cd run/$DATE_START
  mkdir icbc perturb_ic wrf wrf_ens update_bc
  for f in `ls`; do
