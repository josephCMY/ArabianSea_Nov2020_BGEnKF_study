...
module crtm_utils
  ...
  subroutine crtm_input_wrf_col( sat_lon, sat_h, ll )
    ...
    do z=zmax,1,-1
      if(qcloud_sub(ll,z).gt.0.0) then
        ncl = ncl + 1
      endif
      if(qrain_sub(ll,z).gt.0.0) then
        ncl = ncl + 1
      endif
      if(qice_sub(ll,z).gt.0.0) then
        ncl = ncl + 1
      endif
      if(qsnow_sub(ll,z).gt.0.0) then
        ncl = ncl + 1
      endif
      if(qgraup_sub(ll,z).gt.0.0) then
        ncl = ncl + 1
      endif
    enddo
...
module post_process
  ...
  subroutine gather_BT_to_root( BT_sub )
    ...
    if ( my_proc_id == 0 ) then
      ...
      allocate( BT_mask  ( channel_count, total_wrf_cols ) ) 
      BT_mask(:,1:total_wrf_cols) = .true.
    endif
    ...
    if (my_proc_id == 0) then
      unpack_channel_loop: do cc = 1, channel_count
        BT_field3d(cc,:,:) &
          = unpack( BT_all(cc,1:total_wrf_cols), BT_mask, &
                    BT_field3d(cc,:,:) )
