...
!                           ATTRIBUTES: INTENT(IN), OPTIONAL
! ----- Added by Yinghui Lu ------------
!       CloudCoeff_File_ice:Name of the data file containing the cloud optical
!                           properties data for scattering calculations.
!                           (cloud ice)
!                           Available datafiles:
!                           - CloudCoeff.bin  [DEFAULT]
...
!                           ATTRIBUTES: INTENT(IN), OPTIONAL
!
!       CloudCoeff_File_water: Name of the data file containing the cloud optical
!                           properties data for scattering calculations.
!                           (cloud water)
!                           Available datafiles:
!                           - CloudCoeff.bin  [DEFAULT]
...
!                           ATTRIBUTES: INTENT(IN), OPTIONAL
!
!       CloudCoeff_File_rain:    Name of the data file containing the cloud optical
!                           properties data for scattering calculations.
!                           (rain)
!                           Available datafiles:
!                           - CloudCoeff.bin  [DEFAULT]
...
!                           ATTRIBUTES: INTENT(IN), OPTIONAL
!
!       CloudCoeff_File_snow: Name of the data file containing the cloud optical
!                           properties data for scattering calculations.
!                           (snow)
!                           Available datafiles:
!                           - CloudCoeff.bin  [DEFAULT]
...
!                           ATTRIBUTES: INTENT(IN), OPTIONAL
!
!       CloudCoeff_File_graup: Name of the data file containing the cloud optical
!                           properties data for scattering calculations.
!                           (graupel)
!                           Available datafiles:
!                           - CloudCoeff.bin  [DEFAULT]
...
!                           ATTRIBUTES: INTENT(IN), OPTIONAL
!
!       CloudCoeff_File_hail: Name of the data file containing the cloud optical
!                           properties data for scattering calculations.
!                           (hail)
!                           Available datafiles:
!                           - CloudCoeff.bin  [DEFAULT]
...
!------------------------------------------------------------------------------
  ...
  ! ----- Added by Yinghui Lu ------------
    CloudCoeff_File_ice   , &  ! Optional input # added by Yinghui Lu
    CloudCoeff_File_water , &  ! Optional input # added by Yinghui Lu
    CloudCoeff_File_rain  , &  ! Optional input # added by Yinghui Lu
    CloudCoeff_File_snow  , &  ! Optional input # added by Yinghui Lu
    CloudCoeff_File_graup , &  ! Optional input # added by Yinghui Lu
    CloudCoeff_File_hail  ) &  ! Optional input # added by Yinghui Lu
  ! ----- End Added by Yinghui Lu ------------
  RESULT( err_stat )
    ! Arguments
    ...
    ! ----- Added by Yinghui Lu ------------
    CHARACTER(*),      OPTIONAL, INTENT(IN)  :: CloudCoeff_File_ice
    CHARACTER(*),      OPTIONAL, INTENT(IN)  :: CloudCoeff_File_water
    CHARACTER(*),      OPTIONAL, INTENT(IN)  :: CloudCoeff_File_rain
    CHARACTER(*),      OPTIONAL, INTENT(IN)  :: CloudCoeff_File_snow
    CHARACTER(*),      OPTIONAL, INTENT(IN)  :: CloudCoeff_File_graup
    CHARACTER(*),      OPTIONAL, INTENT(IN)  :: CloudCoeff_File_hail
    ! ----- End Added by Yinghui Lu ------------
    ! Function result
    INTEGER :: err_stat
    ! Local parameters
    ...
    ! ----- Added by Yinghui Lu ------------
    CHARACTER(SL) :: Default_CloudCoeff_File_ice
    CHARACTER(SL) :: Default_CloudCoeff_File_water
    CHARACTER(SL) :: Default_CloudCoeff_File_rain
    CHARACTER(SL) :: Default_CloudCoeff_File_snow
    CHARACTER(SL) :: Default_CloudCoeff_File_graup
    CHARACTER(SL) :: Default_CloudCoeff_File_hail
    ...
    ! CloudCoeff_File_xxx is present and not equal to empty string.
    !
    ! CRTM_CloudCoeff_Load() has been modified to take distribution-specific
    ! LUTs. If Default_CloudCoeff_File_xxx is set to empty string ('') or not
    ! given, the corresponding distribution-specific LUT is ignored and the
    ! default CRTM LUT is used instead

    Default_CloudCoeff_File_ice   = ''
    Default_CloudCoeff_File_water = ''
    Default_CloudCoeff_File_rain  = ''
    Default_CloudCoeff_File_snow  = ''
    Default_CloudCoeff_File_graup = ''
    Default_CloudCoeff_File_hail  = ''

    USE_LUT_ice   = .FALSE.
    IF ( PRESENT(CloudCoeff_File_ice  ) ) THEN
      IF (.NOT. (CloudCoeff_File_ice   == '')) USE_LUT_ice   = .TRUE.
    END IF

    USE_LUT_water = .FALSE.
    IF ( PRESENT(CloudCoeff_File_water) ) THEN
      IF (.NOT. (CloudCoeff_File_water == '')) USE_LUT_water = .TRUE.
    END IF

    USE_LUT_rain  = .FALSE.
    IF ( PRESENT(CloudCoeff_File_rain ) ) THEN
      IF (.NOT. (CloudCoeff_File_rain  == '')) USE_LUT_rain  = .TRUE.
    END IF

    USE_LUT_snow  = .FALSE.
    IF ( PRESENT(CloudCoeff_File_snow ) ) THEN
      IF (.NOT. (CloudCoeff_File_snow  == '')) USE_LUT_snow  = .TRUE.
    END IF

    USE_LUT_graup = .FALSE.
    IF ( PRESENT(CloudCoeff_File_graup) ) THEN
      IF (.NOT. (CloudCoeff_File_graup == '')) USE_LUT_graup = .TRUE.
    END IF

    USE_LUT_hail  = .FALSE.
    IF ( PRESENT(CloudCoeff_File_hail ) ) THEN
      IF (.NOT. (CloudCoeff_File_hail  == '')) USE_LUT_hail  = .TRUE.
    END IF
    
    IF ( USE_LUT_ice   ) Default_CloudCoeff_File_ice    = TRIM(ADJUSTL(CloudCoeff_File_ice))
    IF ( USE_LUT_water ) Default_CloudCoeff_File_water  = TRIM(ADJUSTL(CloudCoeff_File_water))
    IF ( USE_LUT_rain  ) Default_CloudCoeff_File_rain   = TRIM(ADJUSTL(CloudCoeff_File_rain))
    IF ( USE_LUT_snow  ) Default_CloudCoeff_File_snow   = TRIM(ADJUSTL(CloudCoeff_File_snow))
    IF ( USE_LUT_graup ) Default_CloudCoeff_File_graup  = TRIM(ADJUSTL(CloudCoeff_File_graup))
    IF ( USE_LUT_hail  ) Default_CloudCoeff_File_hail   = TRIM(ADJUSTL(CloudCoeff_File_hail))
     
    IF ( PRESENT(File_Path) ) THEN
      IF ( USE_LUT_ice   )  Default_CloudCoeff_File_ice    = TRIM(ADJUSTL(File_Path)) // TRIM(Default_CloudCoeff_File_ice)
      IF ( USE_LUT_water )  Default_CloudCoeff_File_water  = TRIM(ADJUSTL(File_Path)) // TRIM(Default_CloudCoeff_File_water)
      IF ( USE_LUT_rain  )  Default_CloudCoeff_File_rain   = TRIM(ADJUSTL(File_Path)) // TRIM(Default_CloudCoeff_File_rain)
      IF ( USE_LUT_snow  )  Default_CloudCoeff_File_snow   = TRIM(ADJUSTL(File_Path)) // TRIM(Default_CloudCoeff_File_snow)
      IF ( USE_LUT_graup )  Default_CloudCoeff_File_graup  = TRIM(ADJUSTL(File_Path)) // TRIM(Default_CloudCoeff_File_graup)
      IF ( USE_LUT_hail  )  Default_CloudCoeff_File_hail   = TRIM(ADJUSTL(File_Path)) // TRIM(Default_CloudCoeff_File_hail)
    END IF

    ! let's make sure Default_CloudCoeff_File_xxx are set to empty strings if
    ! distribution specific LUTs are not used
    
    IF ( .NOT. USE_LUT_ice   )  Default_CloudCoeff_File_ice    = ''
    IF ( .NOT. USE_LUT_water )  Default_CloudCoeff_File_water  = ''
    IF ( .NOT. USE_LUT_rain  )  Default_CloudCoeff_File_rain   = ''
    IF ( .NOT. USE_LUT_snow  )  Default_CloudCoeff_File_snow   = ''
    IF ( .NOT. USE_LUT_graup )  Default_CloudCoeff_File_graup  = ''
    IF ( .NOT. USE_LUT_hail  )  Default_CloudCoeff_File_hail   = ''
    ! ----- End Added by Yinghui Lu ------------
    ...
    ! ----- Added by Yinghui Lu ------------
    ! CRTM_CloudCoeff_Load() has been modified to take distribution-specific
    ! LUTs. If Default_CloudCoeff_File_xxx is set to empty string (''), then the
    ! corresponding distribution-specific LUT is ignored.
    ...
    IF ( Local_Load_CloudCoeff ) THEN
      err_stat = CRTM_CloudCoeff_Load( &
                   ...
                   Process_ID        = Process_ID       , &
                   Output_Process_ID = Output_Process_ID, &
                   Filename_ice   = Default_CloudCoeff_File_ice,   &
                   Filename_water = Default_CloudCoeff_File_water, &
                   Filename_rain  = Default_CloudCoeff_File_rain,  &
                   Filename_snow  = Default_CloudCoeff_File_snow,  &
                   Filename_graup = Default_CloudCoeff_File_graup, &
                   Filename_hail  = Default_CloudCoeff_File_hail )
    ! ----- Added by Yinghui Lu ------------
      IF ( err_stat /= SUCCESS ) THEN
