...
MODULE CRTM_CloudScatter
  ...
  ! ----- Added by Yinghui Lu ------------
  USE CRTM_CloudCoeff,          ONLY: CloudC_ice,  CloudC_water, CloudC_snow, &
                                      CloudC_rain, CloudC_graup, CloudC_hail 
  USE CloudCoeff_Define,        ONLY: CloudCoeff_type, CloudCoeff_Associated
...
!################################################################################
  ...
  SUBROUTINE Get_Cloud_Opt_MW( CloudScatter, &  ! Input  CloudScatter structure
    ...
    INTEGER  :: j, k, l, m
    ! ----- Added by Yinghui Lu ------------
    TYPE(CloudCoeff_type), pointer :: CloudC_used => Null()

    ! determine which CloudC structure should be used
    IF (cloud_type == ICE_CLOUD .AND. CloudCoeff_Associated(CloudC_ice) ) then
      CloudC_used => CloudC_ice
    ELSE IF (cloud_type == WATER_CLOUD .AND. CloudCoeff_Associated(CloudC_water) ) then
      CloudC_used => CloudC_water
    ELSE IF (cloud_type == RAIN_CLOUD .AND. CloudCoeff_Associated(CloudC_rain) ) then
      CloudC_used => CloudC_rain
    ELSE IF (cloud_type == SNOW_CLOUD .AND. CloudCoeff_Associated(CloudC_snow) ) then
      CloudC_used => CloudC_snow
    ELSE IF (cloud_type == GRAUPEL_CLOUD .AND. CloudCoeff_Associated(CloudC_graup) ) then
      CloudC_used => CloudC_graup
    ELSE IF (cloud_type == HAIL_CLOUD .AND. CloudCoeff_Associated(CloudC_hail) ) then
      CloudC_used => CloudC_hail
    ELSE
      CloudC_used => CloudC
    END IF
    ! ----- End Added by Yinghui Lu ------------
    ...
    ! -----------------------------------------
    ! ----- Modified by Yinghui Lu ------------
    ! The modifications here only change CloudC to CloudC_used, which points to
    ! specific CloudC(_xxx) instance.
    csi%f_int = MAX(MIN(CloudC_used%Frequency_MW(CloudC_used%n_MW_Frequencies),Frequency),CloudC_used%Frequency_MW(1))
    CALL find_index(CloudC_used%Frequency_MW, csi%f_int, csi%i1,csi%i2, csi%f_outbound)
    csi%f = CloudC_used%Frequency_MW(csi%i1:csi%i2)

    csi%r_int = MAX(MIN(CloudC_used%Reff_MW(CloudC_used%n_MW_Radii),Reff),CloudC_used%Reff_MW(1))
    CALL find_index(CloudC_used%Reff_MW, csi%r_int, csi%j1,csi%j2, csi%r_outbound)
    csi%r = CloudC_used%Reff_MW(csi%j1:csi%j2)
 
    csi%t_int = MAX(MIN(CloudC_used%Temperature(CloudC_used%n_Temperatures),Temperature),CloudC_used%Temperature(1))
    CALL find_index(CloudC_used%Temperature, csi%t_int, csi%k1,csi%k2, csi%t_outbound)
    csi%t = CloudC_used%Temperature(csi%k1:csi%k2)
    ! ----- End Modified by Yinghui Lu ------------
    ...
    ! ----- Modified by Yinghui Lu ------------
    ! The major difference between distribution-specific LUT (DS_LUT) and CRTM LUT are:
    ! 1. Effective radii of ice and cloud can vary in DS_LUT. As such,
    !    scattering properties needs to be interpolated on effective radii for
    !    cloud ice and cloud water as well.
    ! 2. Each hydrometer type has its own LUT. The index for density dimension for 
    !    solid hydrometer are always set to 1.
    !
    ! We check if a distribution-specific LUTs is used by checking the
    ! association status of corresponding CloudC_xxx. If CloudC_xxx is not
    ! associated, original CRTM LUTs are used, i.e., CloudC_used points to
    ! CloudC (see initialization for CloudC_used above).
      CASE (WATER_CLOUD)
        IF ( .NOT. CloudCoeff_Associated(CloudC_water) ) then
          j = 1
          CALL interp_2D( CloudC_used%ke_L_MW(csi%i1:csi%i2,j,csi%k1:csi%k2), csi%wlp, csi%ylp, ke )
        ELSE
          ! If MP-scheme-specific LUT available, interpolate in 3-D space of
          ! frequency, R_eff, and T
          CALL interp_3D( CloudC_used%ke_L_MW(csi%i1:csi%i2,csi%j1:csi%j2,csi%k1:csi%k2), csi%wlp, csi%xlp, csi%ylp, ke )
        END IF

      ! All 3-D interpolations for rain cloud!
      ! Only modification for rain is the use of the pointer -- Yinghui Lu
      CASE (RAIN_CLOUD)
        CALL interp_3D( CloudC_used%ke_L_MW(csi%i1:csi%i2,csi%j1:csi%j2,csi%k1:csi%k2), csi%wlp, csi%xlp, csi%ylp, ke )
        CALL interp_3D( CloudC_used%w_L_MW(csi%i1:csi%i2,csi%j1:csi%j2,csi%k1:csi%k2) , csi%wlp, csi%xlp, csi%ylp, w  )
        IF ( CloudScatter%n_Phase_Elements > 0 .and. CloudScatter%Include_Scattering ) THEN
          pcoeff(0,1) = POINT_5
          DO m = 1, CloudScatter%n_Phase_Elements
            DO l = 1, CloudScatter%n_Legendre_Terms
              CALL interp_3D( CloudC_used%pcoeff_L_MW(csi%i1:csi%i2,csi%j1:csi%j2,csi%k1:csi%k2,l+CloudScatter%lOffset,m), &
      ...
      ! Each solid hydrometer type has its own DS LUT. As such, the density
      ! dimension is no longer needed. Always set k=1
      CASE (ICE_CLOUD)
        IF (.NOT. CloudCoeff_Associated(CloudC_ice) ) then
          j = 1; k = 3
          CALL interp_1D( CloudC_used%ke_S_MW(csi%i1:csi%i2,j,k), csi%wlp, ke )
        ELSE
          k=1 ! When constructing LUTs, k=1 used for ice
          CALL interp_2D( CloudC_used%ke_S_MW(csi%i1:csi%i2,csi%j1:csi%j2,k), csi%wlp, csi%xlp, ke )
          ! Add scattering for cloud ice too.
          CALL interp_2D( CloudC_used%w_S_MW(csi%i1:csi%i2,csi%j1:csi%j2,k) , csi%wlp, csi%xlp, w  )
          IF (CloudScatter%n_Phase_Elements > 0 .and. CloudScatter%Include_Scattering ) THEN
            pcoeff(0,1) = POINT_5
            DO m = 1, CloudScatter%n_Phase_Elements
              DO l = 1, CloudScatter%n_Legendre_Terms
                CALL interp_2D( CloudC_used%pcoeff_S_MW(csi%i1:csi%i2,csi%j1:csi%j2,k,l+CloudScatter%lOffset,m), &
      ...
      CASE DEFAULT
        ...
        SELECT CASE (Cloud_Type)
          CASE (GRAUPEL_CLOUD)
            IF (.NOT. CloudCoeff_Associated(CloudC_graup) ) then
              k = 2
            ELSE
              k=1
            END IF
          CASE (HAIL_CLOUD)  
            IF (.NOT. CloudCoeff_Associated(CloudC_hail) ) then
              k = 3
            ELSE
              k=1
            END IF
  ...
  SUBROUTINE Get_Cloud_Opt_MW_TL( CloudScatter_TL, &  ! Input  CloudScatter TL structure
    ...
    ! -----------------------------------------
    ! ----- Modified by Yinghui Lu ------------
    ! The modifications here are similar to those in the forward model
    ...
    SELECT CASE (Cloud_Type)
      ...
      CASE (WATER_CLOUD)
        ...
        ! we're only considering Rayleigh scattering here.
          j = 1
          z2 => CloudC_used%ke_L_MW(csi%i1:csi%i2,j,csi%k1:csi%k2)
          CALL interp_2D_TL( z2   , csi%wlp, csi%ylp, &  ! FWD Input
                             z2_TL, wlp_TL , ylp_TL , &  ! TL  Input
                             ke_TL                    )  ! TL  Output
        ELSE
        ! 3-D interpolation if MP-scheme-specific LUT used
          z3 => CloudC_used%ke_L_MW(csi%i1:csi%i2,csi%j1:csi%j2,csi%k1:csi%k2)
      ...
      CASE (RAIN_CLOUD)
        ...
        CALL interp_3D_TL( z3   , csi%wlp, csi%xlp, csi%ylp, &  ! FWD Input
                           ...
                           ke_TL                           )  ! TL  Output
        ! Single scatter albedo
        z3 => CloudC_used%w_L_MW(csi%i1:csi%i2,csi%j1:csi%j2,csi%k1:csi%k2)
        CALL interp_3D_TL( z3   , csi%wlp, csi%xlp, csi%ylp, &  ! FWD Input
                           z3_TL, wlp_TL , xlp_TL , ylp_TL , &  ! TL  Input
                           w_TL                              )  ! TL  Output
        ! Phase matrix coefficients
        IF ( CloudScatter_TL%n_Phase_Elements > 0 .and. CloudScatter_TL%Include_Scattering ) THEN
          ...
          DO m = 1, CloudScatter_TL%n_Phase_Elements
            DO l = 1, CloudScatter_TL%n_Legendre_Terms
              z3 => CloudC_used%pcoeff_L_MW(csi%i1:csi%i2,csi%j1:csi%j2,csi%k1:csi%k2,l+CloudScatter_TL%lOffset,m)
              CALL interp_3D_TL( z3   , csi%wlp, csi%xlp, csi%ylp, &  ! FWD Input
                                 z3_TL, wlp_TL , xlp_TL , ylp_TL , &  ! TL  Input
      ...
      CASE (ICE_CLOUD)
        ...
        ! is only a fn. of frequency for ice cloud
          ke_TL = ZERO
        ELSE
          ! If distribution-specific LUT has multiple effective radii for cloud
          ! ice, need TL output for cloud ice -- similar to snow/graupel/hail
          ...
          k=1
          ! Extinction coefficient
          z2 => CloudC_used%ke_S_MW(csi%i1:csi%i2,csi%j1:csi%j2,k)
          CALL interp_2D_TL( z2   , csi%wlp, csi%xlp, &  ! FWD Input
                             z2_TL, wlp_TL , xlp_TL , &  ! TL  Input
                             ke_TL                    )  ! TL  Output
          ! Single scatter albedo
          z2 => CloudC_used%w_S_MW(csi%i1:csi%i2,csi%j1:csi%j2,k)
          CALL interp_2D_TL( z2   , csi%wlp, csi%xlp, &  ! FWD Input
                             z2_TL, wlp_TL , xlp_TL , &  ! TL  Input
                             w_TL                     )  ! TL  Output
          ! Phase matrix coefficients
          IF ( CloudScatter_TL%n_Phase_Elements > 0 .and. CloudScatter_TL%Include_Scattering ) THEN
            ...
            DO m = 1, CloudScatter_TL%n_Phase_Elements
              DO l = 1, CloudScatter_TL%n_Legendre_Terms
                z2 => CloudC_used%pcoeff_S_MW(csi%i1:csi%i2,csi%j1:csi%j2,k,l+CloudScatter_TL%lOffset,m)
                CALL interp_2D_TL( z2   , csi%wlp, csi%xlp, &  ! FWD Input
                                   z2_TL, wlp_TL , xlp_TL , &  ! TL  Input
                                   pcoeff_TL(l,m)           )  ! TL  Output
              END DO
  ...
  SUBROUTINE Get_Cloud_Opt_MW_AD(CloudScatter_AD, &  ! Input      CloudScatter AD structure
    ...
    ! ----- Modified by Yinghui Lu ------------
      CASE (WATER_CLOUD)
        ...
        ELSE
          ! Perform the AD interpolations when MP-scheme-specific LUT used
          ! Absorption coefficient
          IF( w < ONE ) THEN
            w_AD  = w_AD - ke/(ONE -w) * ke_AD
            ke_AD = ke_AD * (ONE - w) 
      ...
      CASE (RAIN_CLOUD)
        ...
        IF (CloudScatter_AD%n_Phase_Elements > 0 .and. CloudScatter_AD%Include_Scattering ) THEN
          DO m = 1, CloudScatter_AD%n_Phase_Elements
            DO l = 1, CloudScatter_AD%n_Legendre_Terms
              z3 => CloudC_used%pcoeff_L_MW(csi%i1:csi%i2,csi%j1:csi%j2,csi%k1:csi%k2,l+CloudScatter_AD%lOffset,m)
              CALL interp_3D_AD( z3   , csi%wlp, csi%xlp, csi%ylp, &  ! FWD Input
      ...
      ! of frequency for ice cloud
        ...
        ELSE 
          ...
          IF (CloudScatter_AD%n_Phase_Elements > 0 .and. CloudScatter_AD%Include_Scattering ) THEN
            DO m = 1, CloudScatter_AD%n_Phase_Elements
              DO l = 1, CloudScatter_AD%n_Legendre_Terms
                z2 => CloudC_used%pcoeff_S_MW(csi%i1:csi%i2,csi%j1:csi%j2,k,l+CloudScatter_AD%lOffset,m)
