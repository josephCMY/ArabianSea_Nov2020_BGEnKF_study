...
IF ( ASSOCIATED( grid%nca ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",7679,&
...
IF ( ASSOCIATED( grid%w_up ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",7714,&
...
IF ( ASSOCIATED( grid%udr_kf ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",7721,&
...
IF ( ASSOCIATED( grid%tracer_bye ) ) THEN 
 ...
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",15270,&
...
ENDIF
   ...
   END SUBROUTINE init_module_domain
      ...
      SUBROUTINE domain_clockadvance ( grid )
        ...
        IF ( rc /= WRFU_SUCCESS ) THEN
          CALL wrf_error_fatal3("<stdin>",16071,&
