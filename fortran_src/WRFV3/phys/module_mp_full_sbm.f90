     ...
     &     T_NEW(I,K,J),DT,rhocgs(I,K,J),COL,ICEMAX,NKR)
        ...
        IF (T_OLD(I,K,J).GT.213)THEN     
         ...
         DO IKL=1,NCOND
          ...
          IF (DEL2NR.EQ.0)call wrf_error_fatal3("<stdin>",958,&
     ...
     &     kts,kte,nkr)
      ...
      DO KR=1,NKR
       ...
       IF (IFALL.EQ.1)THEN
        ...
        END DO                                                 
        IF(TFALL.GE.1.E10) call wrf_error_fatal3("<stdin>",1977,&
       ...
       DOUBLE PRECISION CHECK,TPN
          CHECK=0.D0
           DO KR=1,NKR
             ...
             IF (CHECK.LT.0) call wrf_error_fatal3("<stdin>",5675,&
	...
	ELSE IF(RI.NE.0.AND.RW*1.E5.LT.RI) THEN
     ...
     &       (DEL1N_2P-RI*DEL2N_2P/PI)*DT+RI*DEL2INT_2P/PI
        ...
        ELSE
          ...
          IF (RW.LE.0) call wrf_error_fatal3("<stdin>",6586,&
"fatal error in module_mp_full_sbm (RW.LE.0), model stop")
          IF (PW.LE.0) call wrf_error_fatal3("<stdin>",6588,&
"fatal error in module_mp_full_sbm (PW.LE.0), model stop")
          IF (RI.LE.0) call wrf_error_fatal3("<stdin>",6590,&
"fatal error in module_mp_full_sbm (RI.LE.0), model stop")
          IF (PI.LE.0) call wrf_error_fatal3("<stdin>",6592,&
"fatal error in module_mp_full_sbm (PI.LE.0), model stop")
	  ...
	  ALFAPX=.5*(ALFA_DP+X_DP)
          IF (ALFAPX.LE.0) call wrf_error_fatal3("<stdin>",6597,&
...
10     continue
       do it = 1,ndiv
         ...
         if (ndiv.gt.10000) call wrf_error_fatal3("<stdin>",8153,&
...
if(pres_SI > 200000.D0.or.pres_SI < 10000.D0) call wrf_error_fatal3("<stdin>",9711,&
...
if(rhoa_SI > 2.D0) call wrf_error_fatal3("<stdin>",9758,&
...
if(FLIQFR_I(IK,I) > 0.D0) then
  ...
  if(IEVAP_ADJUST == 1 ) then
    ...
    if(FLIQFR_I(IK,I) > 1.D0) call wrf_error_fatal3("<stdin>",9906,&
"fatal error in module_mp_full_sbm (FLIQFR_I > 1), model stop 114")
   ...
   XI_MELT_SI(IK,I)=XX_MELT
    ...
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",9969,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",9971,&
"fatal error in module_mp_full_sbm , model stop 138")
  ...
  if(ISUBLIME_ADJUST == 1 ) then
    ...
    if(TS > 273.16) call wrf_error_fatal3("<stdin>",9987,&
"fatal error in module_mp_full_sbm , model stop 13655")
   ...
   (XI_SI(IK,I),rhoa_SI,Q_SUBL,FF2_SI(IK,I))
    ...
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",10016,&
"fatal error in module_mp_full_sbm (QQV < 0), model stop ")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",10018,&
...
IF(FLIQFR_I(IK,I) > FLIQFRAC_LIM) then
  ...
  if(XL_SI(IK).NE.XI_SI(IK,I)) call wrf_error_fatal3("<stdin>",10034,&
...
if(FLIQFR_S(IK) > 0.D0) then
  ...
  if(IEVAP_ADJUST == 1) then
    ...
    if(FLIQFR_S(IK) > 1.D0) call wrf_error_fatal3("<stdin>",10175,&
"fatal error in module_mp_full_sbm , model stop 905")
   ...
   (XS_SI(IK),rhoa_SI,Q_EVAP,FLIQFR_S(IK),FF3_SI(IK))
    ...
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",10225,&
"fatal error in module_mp_full_sbm , model stop 915")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",10227,&
"fatal error in module_mp_full_sbm , model stop 916")
  ...
  if(ISUBLIME_ADJUST == 1) then
    ...
    if(QQV < 0.) call wrf_error_fatal3("<stdin>",10272,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.) call wrf_error_fatal3("<stdin>",10274,&
"fatal error in module_mp_full_sbm , model stop 138")
...
IF(FLIQFR_S(IK) > FLIQFRAC_LIM) then
  ...
  if(XL_SI(IK).NE.XS_SI(IK)) call wrf_error_fatal3("<stdin>",10290,&
...
if(FLIQFR_G(IK) > 0.D0) then
  ...
  if(IEVAP_ADJUST == 1) then
    ...
    if(FLIQFR_G(IK) > 1.D0) call wrf_error_fatal3("<stdin>",10490,&
"fatal error in module_mp_full_sbm , model stop 901")
  ...
  if(ISUBLIME_ADJUST == 1)then
    ...
    if(QQV < 0.D0)  call wrf_error_fatal3("<stdin>",10567,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.D0)   call wrf_error_fatal3("<stdin>",10569,&
"fatal error in module_mp_full_sbm , model stop 138")
...
IF(FLIQFR_G(IK) > FLIQFRAC_LIM) then
  ...
  if(XL_SI(IK).NE.XG_SI(IK)) call wrf_error_fatal3("<stdin>",10583,&
...
if(FLIQFR_H(IK) > 0.D0) then
  ...
  if(IEVAP_ADJUST == 1) then
    ...
    if( FLIQFR_H(IK) > 1.D0) call wrf_error_fatal3("<stdin>",10820,&
"fatal error in module_mp_full_sbm , model stop 906")
   ...
   (XH_SI(IK),rhoa_SI,Q_EVAP,FLIQFR_H(IK),FF5_SI(IK))
    ...
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",10871,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",10873,&
"fatal error in module_mp_full_sbm , model stop 138")
  ...
  if(ISUBLIME_ADJUST == 1) then
    ...
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",10919,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",10921,&
"fatal error in module_mp_full_sbm , model stop 138")
...
IF(FLIQFR_H(IK) > FLIQFRAC_LIM) then
  ...
  if(XL_SI(IK).NE.XH_SI(IK)) call wrf_error_fatal3("<stdin>",10938,&
...
if(FFX < 0.D0) call wrf_error_fatal3("<stdin>",11070,&
"fatal error in module_mp_full_sbm , model stop 136")

if(total_mass_liq < 0.D0) call wrf_error_fatal3("<stdin>",11073,&
"fatal error in module_mp_full_sbm , model stop 137")
if(total_mass_ice < 0.D0) call wrf_error_fatal3("<stdin>",11075,&
"fatal error in module_mp_full_sbm , model stop 140")
if(total_mass < 0.D0) call wrf_error_fatal3("<stdin>",11077,&
...
if(FFX < 0.D0) call wrf_error_fatal3("<stdin>",11125,&
"fatal error in module_mp_full_sbm , model stop 140")

if(total_mass < 0.D0) call wrf_error_fatal3("<stdin>",11128,&
"fatal error in module_mp_full_sbm , model stop 141")
...
if(fx > 100.D0) call wrf_error_fatal3("<stdin>",11460,&
...
if(fx > 100.D0) call wrf_error_fatal3("<stdin>",11487,&
...
if(fx > 100.D0) call wrf_error_fatal3("<stdin>",11513,&
...
if(ix -1 < 1) call wrf_error_fatal3("<stdin>",11591,&
"fatal error in module_mp_full_sbm , model stop 42567")
if(ix  > ix_max) call wrf_error_fatal3("<stdin>",11593,&
...
if(chi_fra > 0.3D0 .and. pc < 75.D0) call wrf_error_fatal3("<stdin>",11608,&
"fatal error in module_mp_full_sbm , model stop 1478")
if(chi_fra > 0.6D0 .and. pc < 90.D0) call wrf_error_fatal3("<stdin>",11610,&
...
if(IKX > NKR.or.IKX < 1) call wrf_error_fatal3("<stdin>",11754,&
...
if( epsil_i < 0.D0) call wrf_error_fatal3("<stdin>",11900,&
...
if(epsil_i > 0.D0) then
  ...
  if((1.D0+epsil_i)/(1.D0-epsil_i).le.0.D0) call wrf_error_fatal3("<stdin>",11914,&
...
if(pres_SI > 200000.D0.or.pres_SI < 10000.D0) call wrf_error_fatal3("<stdin>",12625,&
...
if(rhoa_SI > 2.D0) call wrf_error_fatal3("<stdin>",12669,&
...
"fatal error in module_mp_full_sbm , model stop 9999")
    ...
    if(fm_w < fm_w_soaked) call wrf_error_fatal3("<stdin>",13196,&
"fatal error in module_mp_full_sbm , model stop 9065")
    ...
    if(INEW < INK) then
      ...
      if(fmass_X(INK) < 0.D0) call wrf_error_fatal3("<stdin>",13222,&
"fatal error in module_mp_full_sbm , model stop 8020")
      if(res_mass_shed < 0.D0) call wrf_error_fatal3("<stdin>",13224,&
...
"fatal error in module_mp_full_sbm , model stop 8003")
      ...
      if(FLIQFR_X(INEW+1) < 0.D0.or.FLIQFR_X(INEW+1) > 1.D0) call wrf_error_fatal3("<stdin>",13331,&
"fatal error in module_mp_full_sbm , model stop 8004")
      if(FLIQFR_X(INK) < 0.D0.or.FLIQFR_X(INK) > 1.D0) call wrf_error_fatal3("<stdin>",13333,&
