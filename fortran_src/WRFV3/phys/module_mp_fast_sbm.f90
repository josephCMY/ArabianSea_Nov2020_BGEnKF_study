...
"fatal error in module_mp_fast_sbm (DIV1>DIV2), model stop")
          ...
          IF (DEL2NR.EQ.0)call wrf_error_fatal3("<stdin>",909,&
"fatal error in module_mp_fast_sbm (DEL2NR.EQ.0), model stop")
          ...
          IF (DEL12R.EQ.0)call wrf_error_fatal3("<stdin>",915,&
     ...
     &     kts,kte,nkr)
      ...
      DO KR=1,NKR
       ...
       IF (IFALL.EQ.1)THEN
        ...
        END DO                                                 
        IF(TFALL.GE.1.E10)call wrf_error_fatal3("<stdin>",1459,&
       ...
       DOUBLE PRECISION CHECK,TPN
          CHECK=0.D0
           DO KR=1,NKR
             ...
             IF (CHECK.LT.0) call wrf_error_fatal3("<stdin>",5159,&
	...
	ELSE IF(RI.NE.0.AND.RW*1.E5.LT.RI) THEN
     ...
     &       (DEL1N_2P-RI*DEL2N_2P/PI)*DT+RI*DEL2INT_2P/PI
        ...
        ELSE
          ...
          PI_DP=PI
          PW_DP=PW
          IF (RW.LE.0) call wrf_error_fatal3("<stdin>",6065,&
"fatal error in module_mp_fast_sbm (RW.LE.0), model stop")
          IF (PW.LE.0) call wrf_error_fatal3("<stdin>",6067,&
"fatal error in module_mp_fast_sbm (PW.LE.0), model stop")
          IF (RI.LE.0) call wrf_error_fatal3("<stdin>",6069,&
"fatal error in module_mp_fast_sbm (RI.LE.0), model stop")
          IF (PI.LE.0) call wrf_error_fatal3("<stdin>",6071,&
"fatal error in module_mp_fast_sbm (PI.LE.0), model stop")
	  ...
	  ALFAPX=.5*(ALFA_DP+X_DP)
          IF (ALFAPX.LE.0) call wrf_error_fatal3("<stdin>",6076,&
"fatal error in module_mp_fast_sbm (CHECK.LT.0), model stop")
...
10     continue
       do it = 1,ndiv
         if (ndiv.gt.10000) call wrf_error_fatal3("<stdin>",7630,&
