...
program da_bias_airmass
  ...
  if (lscan) OPEN(12,FORM='UNformatTED')  
...
loop2:&
   ...
   if (check_limb) then
    ...
    do j=1, tovs%nchan
     if ( tovs%qc_flag(j) == 1 ) then 
        ...
        ybar(j) = ybar(j) + vec_dep(j)             
        ycov(j) = ycov(j) + vec_dep(j)*vec_dep(j)  

        do i=1, tovs%npred                         
           xbar(j,i) = xbar(j,i) + pred(i)         
          xycov(j,i) = xycov(j,i) + vec_dep(j)*pred(i) 
          do ii=1, tovs%npred
            xcov(j,i,ii) = xcov(j,i,ii) + pred(i)*pred(ii) 
          end do
        end do
  ...
  390 format (1X,I5,4F15.2)
    ...
    coef_day   = real(MOD(CDATE,10000)/100)                  
