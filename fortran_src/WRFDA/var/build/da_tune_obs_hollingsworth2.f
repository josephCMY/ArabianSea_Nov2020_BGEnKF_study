...
program da_tune_obs_hollingsworth2
   ...
   real, parameter           :: max_distance = 5000      
   ...
   real                      :: bin_start_p(1:num_bins_p)    
   real                      :: obs_err_used(1:num_bins_p)   
...
subroutine da_obs_stats( num_times, obs )
   ...
   if ( nobs > 0 ) then
      ...
      obs % stdv_omb = sumobs2 / real(nobs) 
   ...
   do n1 = 1, num_stations
      ...
      do n2 = n1, num_stations
            ...
            dist = acos( cos(colat1) * cos(colat2) + &
                         sin(colat1) * sin(colat2) * cos(londiff) ) 
            dis(n1,n2) = earth_radius * dist                
         end if
