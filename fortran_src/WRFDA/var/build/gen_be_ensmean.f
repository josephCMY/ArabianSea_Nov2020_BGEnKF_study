...
!
   ...
   integer, parameter    :: max_num_dims = 4          
   ...
   integer               :: dimids(1:10)              
   integer               :: dims(1:max_num_dims)      
   integer               :: istart(1:max_num_dims)    
   integer               :: iend(1:max_num_dims)      

   real (kind=4), allocatable     :: data_r(:,:,:)             
   real (kind=4), allocatable     :: data_r_mean(:,:,:)        
   real (kind=4), allocatable     :: data_r_vari(:,:,:)        
   ...
   if ( rcode /= 0) then
      ...
      call da_error("gen_be_ensmean.b",90,message(1:1))
   end if
   ...
   if ( rcode /= 0) then
      write(UNIT=message(1),FMT='(A,A)') &
         ' Error opening netcdf file ', input_file(1:length)
      call da_error("gen_be_ensmean.b",100,message(1:1))
   end if
      ...
      do member = 1, num_members
         ...
         if ( member == 1 ) then
            ...
            if ( rcode /= 0 ) then
               ...
               call da_error("gen_be_ensmean.b",128,message(1:1))
             end if
             ...
             if ( ivtype == 5 ) then
                ...
                call da_error("gen_be_ensmean.b",154,message(1:1))
             end if
