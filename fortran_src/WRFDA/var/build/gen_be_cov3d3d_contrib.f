...
program gen_be_cov3d3d_contrib
   ...
   allocate( regcoeff_field1_field2(1:nk,1:nk,1:num_bins2d) )
   ...
   regcoeff_field1_field2 = regcoeff_psi_t
   exit
   end if
   ...
   if(trim(adjustl(variable)) == trim(adjustl(regcoeff_var)) )then
   regcoeff_field1_field2 = regcoeff_psi_rh 
   exit
   end if
   case ('regcoeff_chi_u_t')
   read (iunit) regcoeff_chi_u_t
   if(trim(adjustl(variable)) == trim(adjustl(regcoeff_var)) ) then
   regcoeff_field1_field2 = regcoeff_chi_u_t
   exit
   end if
   ...
   if(trim(adjustl(variable)) == trim(adjustl(regcoeff_var)) )then
   regcoeff_field1_field2 = regcoeff_chi_u_rh
   exit
   end if
   case ('regcoeff_t_u_rh')
   read (iunit) regcoeff_t_u_rh
   if(trim(adjustl(variable)) == trim(adjustl(regcoeff_var)) )then
   regcoeff_field1_field2 = regcoeff_t_u_rh
   exit
   end if
      ...
      do member = 1, ne
         ...
         do k = 1, nk
            do j = 1, nj
               do i = 1, ni
                  b = bin2d(i,j)
                  field2_balanced(i,j,k) = SUM(regcoeff_field1_field2(k,1:nk,b) * field1(i,j,1:nk))
               end do
            end do
         end do
...
end program gen_be_cov3d3d_contrib
