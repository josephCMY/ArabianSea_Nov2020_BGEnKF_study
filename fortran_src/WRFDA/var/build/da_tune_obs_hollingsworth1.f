...
subroutine da_write_data( lev, current_time, ounit, info, p, field )
   ...
   if ( info % time == current_time ) then  
   ...
   do       
      ...
      elseif ( index( ob_name,'polaramv') > 0 ) then
         ...
         do n = num_obs_polaramv + 1, num_obs_polaramv + num_obs
            ...
            do k = 1, num_levs
            read(y_unit,'(2i8,a5,2f9.2,f17.7,5(2f17.7,i8,2f17.7))') &
            ndum, kdum, ob % polaramv(n) % info % id, &        
                        ob % polaramv(n) % info % lat, &       
                        ob % polaramv(n) % info % lon, &       
                        ob % polaramv(n) % pressure, &         
                        ob % polaramv(n) % u, &                
                        ob % polaramv(n) % v
      ...
      elseif ( index( ob_name,'geoamv') > 0 ) then
         ...
         do n = num_obs_geoamv + 1, num_obs_geoamv + num_obs
            ...
            do k = 1, num_levs
            read(y_unit,'(2i8,a5,2f9.2,f17.7,5(2f17.7,i8,2f17.7))') &
            ndum, kdum, ob % geoamv(n) % info % id,  &       
                        ob % geoamv(n) % info % lat, &       
                        ob % geoamv(n) % info % lon, &       
                        ob % geoamv(n) % pressure,   &       
                        ob % geoamv(n) % u, &                
                        ob % geoamv(n) % v
            end do
      ...
      elseif ( index( ob_name,'sound') > 0 ) then
         ...
         do n = num_obs_sound + 1, num_obs_sound + num_obs
            ...
            do k = 1, num_levs
               read(y_unit,'(2i8,a5,2f9.2,f17.7,5(2f17.7,i8,2f17.7))') &
               ndum, kdum, ob % sound(n) % info % id, &     
                        ob % sound(n) % info % lat, &       
                        ob % sound(n) % info % lon, &       
                        ob % sound(n) % pressure(k), &      
                        ob % sound(n) % u(k), &             
                        ob % sound(n) % v(k), &             
                        ob % sound(n) % t(k), &             
                        ob % sound(n) % q(k)                
      ...
      elseif ( index( ob_name,'airsr') > 0 ) then
         ...
         do n = num_obs_airsr + 1, num_obs_airsr + num_obs
            ...
            do k = 1, num_levs
               read(y_unit,'(2i8,a5,2f9.2,f17.7,5(2f17.7,i8,2f17.7))') &
               ndum, kdum, ob % airsr(n) % info % id, &     
                        ob % airsr(n) % info % lat, &       
                        ob % airsr(n) % info % lon, &       
                        ob % airsr(n) % pressure(k), &      
                        ob % airsr(n) % t(k), &             
                        ob % airsr(n) % q(k)                
            end do
      ...
      elseif ( index( ob_name,'airep') > 0 ) then
         ...
         do n = num_obs_airep + 1, num_obs_airep + num_obs
            ...
            do k = 1, num_levs
               read(y_unit,'(2i8,a5,2f9.2,f17.7,5(2f17.7,i8,2f17.7))') &
               ndum, kdum, ob % airep(n) % info % id, &     
                        ob % airep(n) % info % lat, &       
                        ob % airep(n) % info % lon, &       
                        ob % airep(n) % pressure(k), &      
                        ob % airep(n) % u(k), &             
                        ob % airep(n) % v(k), &             
                        ob % airep(n) % t(k)                
            end do
      ...
      elseif ( index( ob_name,'pilot') > 0 ) then
         ...
         do n = num_obs_pilot + 1, num_obs_pilot + num_obs
            ...
            do k = 1, num_levs
               read(y_unit,'(2i8,a5,2f9.2,f17.7,5(2f17.7,i8,2f17.7))') &
               ndum, kdum, ob % pilot(n) % info % id, &     
                        ob % pilot(n) % info % lat, &       
                        ob % pilot(n) % info % lon, &       
                        ob % pilot(n) % pressure(k), &      
                        ob % pilot(n) % u(k), &             
                        ob % pilot(n) % v(k)
            end do
      ...
      elseif ( index( ob_name,'satem') > 0 ) then
         ...
         do n = num_obs_satem + 1, num_obs_satem + num_obs
            ...
            do k = 1, num_levs
               read(y_unit,'(2i8,a5,2f9.2,f17.7,5(2f17.7,i8,2f17.7))') &
               ndum, kdum, ob % satem(n) % info % id, &     
                        ob % satem(n) % info % lat, &       
                        ob % satem(n) % info % lon, &       
                        ob % satem(n) % pressure(k), &      
     ...
     elseif ( index( ob_name,'sonde_sfc') > 0 ) then
         ...
         do n = num_obs_sonde_sfc + 1, num_obs_sonde_sfc + num_obs
            ...
            do k = 1, num_levs
            read(y_unit,'(2i8,a5,2f9.2,f17.7,5(2f17.7,i8,2f17.7))') &
            ndum, kdum, ob % sonde_sfc(n) % info % id, &        
                        ob % sonde_sfc(n) % info % lat, &       
                        ob % sonde_sfc(n) % info % lon, &       
                        ob % sonde_sfc(n) % pressure, &         
                        ob % sonde_sfc(n) % u, &                
                        ob % sonde_sfc(n) % v, &                
                        ob % sonde_sfc(n) % t, &                
                        ob % sonde_sfc(n) % p, &                
      ...
      elseif ( index( ob_name,'profiler') > 0 ) then
         ...
         do n = num_obs_profiler + 1, num_obs_profiler + num_obs
            ...
            do k = 1, num_levs
               read(y_unit,'(2i8,a5,2f9.2,f17.7,5(2f17.7,i8,2f17.7))') &
               ndum, kdum, ob % profiler(n) % info % id, &     
                        ob % profiler(n) % info % lat,   &     
                        ob % profiler(n) % info % lon,   &     
                        ob % profiler(n) % pressure(k),  &     
                        ob % profiler(n) % u(k), &             
                        ob % profiler(n) % v(k)
            end do
      ...
      elseif ( index( ob_name,'gpsref') > 0 ) then
         ...
         do n = num_obs_gpsref + 1, num_obs_gpsref + num_obs
            ...
            do k = 1, num_levs
               read(y_unit,'(2i8,a5,2f9.2,f17.7,5(2f17.7,i8,2f17.7))') &
               ndum, kdum, ob % gpsref(n) % info % id, &     
                        ob % gpsref(n) % info % lat,   &     
                        ob % gpsref(n) % info % lon,   &     
                        ob % gpsref(n) % pressure(k),  &     
                        ob % gpsref(n) % ref(k)              
            end do
...
subroutine da_calc_stats_sound( num_obs, sound )
   ...
   integer, intent(in)           :: num_obs               
   type (sound_type), intent(in) :: sound(1:num_obs)      
...
subroutine da_calc_stats_airsr( num_obs, airsr )
   ...
   integer, intent(in)           :: num_obs               
   type (airsr_type), intent(in) :: airsr(1:num_obs)      
...
subroutine da_calc_stats_bogus( num_obs, bogus )
   ...
   integer, intent(in)           :: num_obs               
   type (bogus_type), intent(in) :: bogus(1:num_obs)      
...
subroutine da_calc_stats_airep( num_obs, airep )
   ...
   integer, intent(in)           :: num_obs               
   type (airep_type), intent(in) :: airep(1:num_obs)      
...
subroutine da_calc_stats_pilot( num_obs, pilot )
   ...
   integer, intent(in)           :: num_obs               
   type (pilot_type), intent(in) :: pilot(1:num_obs)      
...
subroutine da_calc_stats_profiler( num_obs, profiler )
   ...
   integer, intent(in)           :: num_obs               
   type (pilot_type), intent(in) :: profiler(1:num_obs)      
...
subroutine da_calc_stats_gpsref( num_obs, gpsref )
   ...
   integer, intent(in)           :: num_obs               
   type (gpsref_type), intent(in) :: gpsref(1:num_obs)      
...
subroutine da_calc_stats_satem( num_obs, satem )
   ...
   integer, intent(in)           :: num_obs               
   type (satem_type), intent(in) :: satem(1:num_obs)      
...
subroutine da_calc_stats_ssmt1( num_obs, ssmt1 )
   ...
   integer, intent(in)           :: num_obs               
   type (ssmt1_type), intent(in) :: ssmt1(1:num_obs)      
...
subroutine da_calc_stats_ssmt2( num_obs, ssmt2 )
   ...
   integer, intent(in)           :: num_obs               
   type (ssmt2_type), intent(in) :: ssmt2(1:num_obs)      
