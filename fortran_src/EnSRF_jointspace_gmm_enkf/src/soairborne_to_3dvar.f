...
!----------------------------------------------------------------
  ...
  integer            :: unit
  integer            :: ie, ix, jx, kx, kxs, ntot, itot, i, j, k, len, nr
...
! just for radar
  ...
  integer   :: iob, num, iunit, lev, nu, i1, j1
  real      :: di, di1, dj, dj1
...
! Get argc 
  if (iargc() .lt. 1) then
     ...
     write(6,*) ' example: verify.exe verify_ahgx_vhgxsr1_nx10z12_err3_0212_d02'
     stop
  else
...
! calculate radar rv obser points
  ...
  if ( use_airborne_rv) num_rv = num_rv + raw%airborne%num
...
! get obser point info
  ...
  iobs = 0
  write(char_total_Radar(15:19),'(i5)')raw%radar_stn_num+raw%airborne%num-1
  ...
  if ( use_airborne_rv ) then
      ...
      do num  = 1, raw%airborne%num

         write(99,'(a)')'  '

               char_ned( 1:40) = 'KNP3   NOAA P3radar                     '
               char_ned(41:80) = '                                        '
         write(char_ned(20:27),'(f8.3)')raw%airborne%lon(num)
         write(char_ned(30:37),'(f8.3)')raw%airborne%lat(num)
         write(char_ned(40:47),'(f8.3)')raw%airborne%height(num)
         write(char_ned(50:68),'( a19)')times(1:19)
         write(char_ned(69:74),'(  i6)')1
         write(char_ned(75:80),'(  i6)')1
         write(99,'(a80)')char_ned(1:80)
         ...
         write(99,'(a)')'  '
              ...
              iobs = iobs + 1
              ob  ( iobs ) = raw%airborne%rv(num)
              obs_ii(iobs) = raw%airborne%ii(num)
              obs_jj(iobs) = raw%airborne%jj(num)
              obs_hh(iobs) = raw%airborne%hh(num)*1000.
              obs_az(iobs) = raw%airborne%azim(num)
              obs_el(iobs) = raw%airborne%elev(num)
              obs_ra(iobs) = raw%airborne%range(num)
