...
!-- use_microwave
   use_microwave      = .false.
   datathin_microwave = 999
   hroi_microwave     = 999
   vroi_microwave     = 999
   aoei_microwave     = .false.
   use_slant_path     = .false.
...
!-- enkf_parameter
   ...
   read ( unit = namelist_unit, nml = microwave, iostat = iost )
   if( iost .ne. 0 ) then
       write(*,*)'microwave, please check it.'
       stop 'read_namelist microwave'
   endif
   ...
   print_namelist_selections: if ( my_proc_id == 0 ) then
      write(6, *)'   '
      write(6, *)'---------------------------------------------------'
   ...
   endif print_namelist_selections
...
!========================================================================================
subroutine get_wrf_info(wrf_file, ix, jx, kx, kxs, times, proj)
...
!-----------------------------------------------------------------------------
   ...
   integer, intent(out)            :: ix, jx, kx, kxs
...
!-----------------------------------------------------------------------------
   ...
   call get_ij ( wrf_file, ix, jx, kx, kxs )
   if ( my_proc_id == 0 ) write(6,'(4(a7,i3))')"   ix :",ix,"   jx :",jx,"   kx:",kx,"   kxs:",kxs
...
!====================================================================
! Subroutine to find indices of the wrf enkfvar
subroutine find_enkfvar_indices(nv)
  ...
  ! Now seek out these variables
  varname_loop: do vv = 1, nv
    ! Load variable name
    varname = enkfvar(vv)
    ! Check against list of variables
    ! Dealing with moisture
    if ( trim(varname) == 'QVAPOR'    ) ind_qvapor  = vv
    if ( trim(varname) == 'QCLOUD'    ) ind_qcloud  = vv
    if ( trim(varname) == 'QRAIN'     ) ind_qrain   = vv
    if ( trim(varname) == 'QICE'      ) ind_qice    = vv
    if ( trim(varname) == 'QSNOW'     ) ind_qsnow   = vv
    if ( trim(varname) == 'QGRAUP'    ) ind_qgraup  = vv
    ! Dealing with thermodynamic fields
    if ( trim(varname) == 'P'         ) ind_p       = vv
    if ( trim(varname) == 'PB'        ) ind_pb      = vv
    if ( trim(varname) == 'TSK'       ) ind_tsk     = vv
    if ( trim(varname) == 'T'         ) ind_t       = vv
    if ( trim(varname) == 'T2'        ) ind_t2      = vv
    if ( trim(varname) == 'TH2'       ) ind_th2     = vv
    if ( trim(varname) == 'Q2'        ) ind_q2      = vv
    if ( trim(varname) == 'PSFC'      ) ind_psfc    = vv
    ! Dealing with dynamic fields
    if ( trim(varname) == 'PH'        ) ind_ph      = vv
    if ( trim(varname) == 'PHB'       ) ind_phb     = vv
    if ( trim(varname) == 'HGT'       ) ind_hgt     = vv
    if ( trim(varname) == 'U'         ) ind_u       = vv
    if ( trim(varname) == 'V'         ) ind_v       = vv
    if ( trim(varname) == 'W'         ) ind_w       = vv
    if ( trim(varname) == 'U10'       ) ind_u10     = vv
    if ( trim(varname) == 'V10'       ) ind_v10     = vv
    if ( trim(varname) == 'MU'        ) ind_mu      = vv
    if ( trim(varname) == 'MUB'       ) ind_mub     = vv
  enddo varname_loop

end subroutine find_enkfvar_indices

!====================================================================
! Subroutine to ensure that all the necessary variables are in enkfvar
! Many observation operators need these variables
subroutine supplement_enkfvar(nv)
  ...
  integer :: m, v
  character(len=10), dimension(30) :: needvars

  ! Variables needed for various observation operators
  write( needvars(1 ), '(a)') 'QVAPOR    '
  write( needvars(2 ), '(a)') 'QCLOUD    '
  write( needvars(3 ), '(a)') 'QRAIN     '
  write( needvars(4 ), '(a)') 'QICE      '
  write( needvars(5 ), '(a)') 'QSNOW     '
  write( needvars(6 ), '(a)') 'QGRAUP    '
  write( needvars(7 ), '(a)') 'P         '
  write( needvars(8 ), '(a)') 'PB        '
  write( needvars(9 ), '(a)') 'TSK       '
  write( needvars(10), '(a)') 'T         '
  write( needvars(11), '(a)') 'T2        '
  write( needvars(12), '(a)') 'TH2       '
  write( needvars(13), '(a)') 'Q2        '
  write( needvars(14), '(a)') 'PSFC      '
  write( needvars(15), '(a)') 'PH        '
  write( needvars(16), '(a)') 'PHB       '
  write( needvars(17), '(a)') 'HGT       '
  write( needvars(18), '(a)') 'U         '
  write( needvars(19), '(a)') 'V         '
  write( needvars(20), '(a)') 'W         '
  write( needvars(21), '(a)') 'U10       '
  write( needvars(22), '(a)') 'V10       '
  write( needvars(23), '(a)') 'MU        '
  write( needvars(24), '(a)') 'MUB       '

  ! Check if enkfvar has these variables
  loop_needvars: do v = 1,24

    m = 1
    ! Loop over all variables. Breaks if needvar(v) is in enkfvar(m)
    loop_enkfvar: do while (  m .le. nv )
      if ( trim( needvars(v) ) ==  trim(enkfvar(m)) ) exit
      m = m+1
    enddo loop_enkfvar

    call MPI_BARRIER(comm, ierr)
    ! insert variable into enkfvar if missing
    if ( trim(enkfvar(m)) .ne. trim(needvars(v)) ) then
      enkfvar(nv+1) = needvars(v)
      nv = nv+1
    endif

  enddo loop_needvars

end subroutine supplement_enkfvar



!========================================================================================
subroutine rd_truth_wrf(wrf_file,ix,jx,kx,kxs,ni,nj,nk,nv,nm,gid,sid,iid,jid,x)
   use namelist_define
   use wrf_tools
   use mpi_module
   implicit none
   ...
   character (len=10), intent(in) :: wrf_file
   integer, intent(in)  :: ix, jx, kx,kxs,ni,nj,nk,nv,nm,gid,sid,iid,jid
   real, dimension(ni,nj,nk,nv), intent(out) :: x
   integer  :: ii, jj, kk, m, istart,iend,jstart,jend
   real, allocatable, dimension(:,:,:) :: dat3d

   do m=1,nv
      call wrf_var_dimension ( wrf_file, enkfvar(m), ix, jx, kx,kxs, ii, jj, kk )
      istart=iid*ni+1
      iend=(iid+1)*ni
      jstart=jid*nj+1
      jend=(jid+1)*nj
...
!==================================================================
! Ensemble reader
! Written by: M-Y. Chan (Oct 2020)
!==================================================================
subroutine read_ensemble_bcast(i_unit)
  ...
  integer :: gid, sid, iid, jid
  integer :: ii,jj,kk,ie,n,m,ist,ied,jst,jed, tid, n2
  character(len=10) :: wrf_file
  real, dimension(ix+1, jx+1, kx+1) :: dat3d_bcast
  real, allocatable, dimension(:,:,:):: dat3d

  ! Variables relating to passing decomposed fields
  integer :: iid2, jid2, ist2, ied2, jst2, jed2, sid2, gid2, tid2
  ...
  ! rename my_proc_id to tid
  tid = my_proc_id
  ! Find sid, gid, iid, jid
  ...
  ! Reading procedure for each variable
  ! ------------------------------------
  ! 1) First num_ens processes reads in the relevant ensemble files.
  ! 2) Past information to all processes
  !

  ! Loop over all enkfvar fields
  wrf_field_loop: do m = 1,nv

    ! Temporary wrf_file. Used to figure out the size of variable dimensions
    ...
    jst=jid*nj+1
    jed=(jid+1)*nj
    if(iid==(nicpu-1)) ied=ii
    if(jid==(njcpu-1)) jed=jj


    ! Part 1) First num_ens procs read wrf files
    ! ------------------------------------------
    file_reading_procs: if (tid < numbers_en ) then
      ! Prepare holding array
      allocate(dat3d(ii,jj,kk))
      dat3d=0.
      write(wrf_file,'(a5,i5.5)') 'fort.',i_unit+tid+1
    ...
    endif file_reading_procs

    call MPI_BARRIER(comm, ierr)
!    if( my_proc_id == 0) write(*,*) 'Imported ',enkfvar(m)


    ! Part 2) Communicate data to all processes
    ! ---------------------------------------------
    ! Will communicate via bcasts for simplicity
    ! We can use some send-receive procedures to save RAM, but might take a long time.
    bcast_ens_loop: do n2 =1,numbers_en

      ! Initialize array
      dat3d_bcast=0.

      ! Reader proc loads data into bcast array
      if ( tid+1 == n2 ) dat3d_bcast(1:ii,1:jj,1:kk) =dat3d_bcast(1:ii,1:jj,1:kk) + dat3d

      ! Bcast step from tid = n2-1
      call MPI_BCAST( dat3d_bcast, (ix+1)*(jx+1)*(kx+1), MPI_REAL, n2-1, comm, ierr)
      call MPI_BARRIER( comm, ierr )
!      if( my_proc_id == 0) write(*,*) 'Got past bcast'

      ! Each process now decides which data to read into their slabs
      do n = 1, nm
        ie = (n-1)*nmcpu + gid + 1
        if ( ie == n2) then
          ! Read in the slab data
          x(1:ied-ist+1, 1:jed-jst+1,1:kk,m,n) = dat3d_bcast(ist:ied,jst:jed,1:kk)

          ! Read in boundary data
          if (ist>1) &
            x_we_bdy(1,:,:,m,n) = dat3d_bcast(   ist-1,  jst:jed, :)  ! Western boundary
          if (ied+1<=ii) &
            x_we_bdy(2,:,:,m,n) = dat3d_bcast(   ied+1,  jst:jed, :)  ! Northern boundary
          if (jst>1) &
            x_sn_bdy(1,:,:,m,n) = dat3d_bcast( ist:ied,    jst-1, :)  ! Sourthern boundary
          if (jed+1<=jj) &
            x_sn_bdy(2,:,:,m,n) = dat3d_bcast( ist:ied,    jed+1, :)  ! Northern boundary

          ! Grab corners
          if ( ist > 1 .and. jst > 1 ) &
            x_crnrs(1,1,:,m,n) = dat3d_bcast( ist-1, jst-1, : ) ! West-south corner
          if ( ist > 1 .and. jed+1 <= jj ) &
            x_crnrs(1,2,:,m,n) = dat3d_bcast( ist-1, jed+1, : ) ! West-north corner
          if ( ied+1 <= ii .and. jst > 1 ) &
            x_crnrs(2,1,:,m,n) = dat3d_bcast( ied+1, jst-1, : ) ! East-south corner
          if ( ied+1 <= ii .and. jed+1 <= jj ) &
            x_crnrs(2,2,:,m,n) = dat3d_bcast( ied+1, jed+1, : ) ! East-north corner

        endif
      enddo
    enddo bcast_ens_loop

    ! Cleaning up memory
    call MPI_BARRIER( comm, ierr)
    if ( tid < numbers_en ) then
      deallocate( dat3d )
!!      write(*,*) tid,' dellocated dat3d'
    endif
!    deallocate( dat3d_bcast )
!    call MPI_BARRIER( comm, ierr )
!    if( tid == 0) write(*,*)'Finished processing ',enkfvar(m)

  enddo wrf_field_loop

end subroutine read_ensemble_bcast


!==================================================================
! Fast ensemble reader using scatter
!
! Written by: M-Y. Chan (Oct 2020)
!-----------------------------------------------------------------
! Used if the number of processes is larger than number of members
!==================================================================
subroutine read_ensemble_scat(i_unit)
  ...
  integer, intent(in)  :: i_unit
  integer :: gid, sid, iid, jid, st_ind, ed_ind, root_proc, r_comm, root
  integer :: ii,jj,kk,ie,n,m,ist,ied,jst,jed, tid, n2, ie2
  character(len=10) :: wrf_file
  real, dimension(ix+1, jx+1, kx+1) :: dat3d_bcast
  real, allocatable, dimension(:,:,:):: dat3d
  real, allocatable, dimension(:,:,:,:) :: dat4d

  ! Variables relating to passing decomposed fields
  integer :: iid2, jid2, ist2, ied2, jst2, jed2, sid2, gid2, tid2
  integer, dimension( numbers_en ) :: read_procs
  integer :: proc_interval, flag_recv
  real, allocatable, dimension(:) :: scatbuffer, recvbuffer
  real, allocatable, dimension( :,:,:,: ) :: loc4d
  ...
  ! Determine which processes to do the reading
  ! -------------------------------------------
  ! Note that reader processes are spread out so as to spread the memory usage
  ! across nodes
  read_procs = -9999
  proc_interval = int( nprocs / numbers_en)
  do n = 0, numbers_en-1
    read_procs(n+1) = n * proc_interval
  enddo


  ! Reader processes load full WRF files
  ! ------------------------------------
  load_file: if ( mod( tid, proc_interval ) == 0 &
                 .and. int(tid/proc_interval)<numbers_en+1 ) then

    ! Allocate array to hold the WRF file fields
    ! Note the +2 in the first 2 dimensions. Adding a border of zero values
    ! around the domain for ease of handling boundaries
    allocate( dat4d( ni*nicpu+2, nj*njcpu+2, nk, nv ) )
    dat4d = 0.

    ! Determine which file to read
    write( wrf_file,'(a5,i5.5)') 'fort.',i_unit+ int(tid/proc_interval) + 1

    ! Load WRF fields into holding array
    wrf_field_loop: do m = 1 , nv
      ...
      ! load variable into the holding array
      dat4d(2:ii+1, 2:jj+1, 1:kk, m ) = dat3d(:,:,:)
  ...
  ! Proceed to scatter slab data to all procs
  ! ------------------------------------------

  ! Load data into scattering buffer
  prep_slab_scatter: if ( mod( tid, proc_interval ) == 0   &
                         .and. int(tid/proc_interval)<numbers_en+1   ) then

    ! Allocate scattering buffer
    allocate( scatbuffer( (ni+2)*nicpu * (nj+2)*njcpu * nk * nv ) )
    scatbuffer = 0.

    ! Now fill scatter buffer
    do sid2 = 0, nicpu*njcpu-1

      ! Determine start index in scatter buffer
      st_ind = sid2 * (ni+2)*(nj+2)*nk*nv+1
      ed_ind = st_ind-1 + (ni+2)*(nj+2)*nk*nv

      ! Determine iid and jid corresponding to sid2
      iid2 = mod( sid2, nicpu )
      jid2 = int( sid2/ nicpu )

      ! Determine indices of expanded slab in term of the dat4d.
      ! Note that dat4d has borders of zeros aronund the domain.
      ist2=iid2*ni+1
      ied2=iid2*ni+ni+2
      jst2=jid2*nj+1
      jed2=jid2*nj+nj+2

      ! Load scattering buffer
      scatbuffer( st_ind:ed_ind ) &
      = reshape( dat4d( ist2:ied2, jst2:jed2, :, : ), (/(ni+2)*(nj+2)*nk*nv/) )

      !if (tid == 0) write(*,*) ed_ind-st_ind+1, (ni+2)*(nj+2)*nk*nv

    enddo
  endif prep_slab_scatter


  ! Now scatter slab data
  call MPI_BARRIER( comm, ierr )
  allocate( recvbuffer((ni+2)*(nj+2)*nk*nv) )
  ! Iterate over all members in ensemble
  scatter_members: do ie = 1, numbers_en

    ! Select root process
    root_proc = read_procs(ie)

    ! Blank out buffer
    recvbuffer = 0.

    ! Separate relevant and irrelevant receiving procs
    flag_recv = 0
    do n = 1, nm
      ie2 = (n-1)*nmcpu + gid+1
      if (ie2 == ie) then
    ...
    enddo
    ! Special handling for root proc
    if ( tid==root_proc) flag_recv = 1


    ! Generate communication group for receiving procs
    ! Process rank is determined by sid
    call MPI_COMM_SPLIT( comm, flag_recv, sid, r_comm, ierr )

    ! Perform scatter on all relevant scattering procs and read data into
    ! relevant variables
    if ( flag_recv == 1 ) then

      if ( tid == root_proc ) call MPI_COMM_RANK( r_comm, root, ierr)
      call MPI_BCAST( root, 1, MPI_INTEGER, root_proc, comm, ierr )

      call MPI_SCATTER( scatbuffer, (ni+2)*(nj+2)*nk*nv, MPI_REAL, &
                        recvbuffer, (ni+2)*(nj+2)*nk*nv, MPI_REAL, &
                        root, r_comm, ierr )
      n = int((ie - gid - 1)/nmcpu) + 1

      ! Interpret receiver buffer
      allocate( loc4d( ni+2, nj+2, nk, nv ) ); loc4d = 0.
      loc4d = reshape( recvbuffer, (/ni+2, nj+2, nk, nv/) )

      ! Loading slab
      x(:,:,:,:,n) = loc4d(2:ni+1, 2:nj+1, :,:)

      ! Loading the boundaries
      if (iid > 0      ) x_we_bdy(1,:,:,:,n) = loc4d(1     , 2:nj+1,:,:)
      if (iid < nicpu-1) x_we_bdy(2,:,:,:,n) = loc4d(ni+2  , 2:nj+1,:,:)
      if (jid > 0      ) x_sn_bdy(1,:,:,:,n) = loc4d(2:ni+1, 1     ,:,:)
      if (jid < njcpu-1) x_sn_bdy(2,:,:,:,n) = loc4d(2:ni+1, nj+2  ,:,:)

      ! Load the corners
      if ( iid > 0       .and. jid > 0      ) x_crnrs(1,1,:,:,n) = loc4d(1   ,1   ,:,:)
      if ( iid > 0       .and. jid < njcpu-1) x_crnrs(1,2,:,:,n) = loc4d(1   ,nj+2,:,:)
      if ( iid < nicpu-1 .and. jid > 0      ) x_crnrs(2,1,:,:,n) = loc4d(ni+2,1   ,:,:)
      if ( iid < nicpu-1 .and. jid < njcpu-1) x_crnrs(2,2,:,:,n) = loc4d(ni+2,nj+2,:,:)

      ! Deallocate interpreter buffer
      deallocate(loc4d)

    endif

    ! Kill off the communication group
    call MPI_BARRIER(comm, ierr )
    call MPI_COMM_FREE( r_comm, ierr )
    call MPI_BARRIER( comm, ierr)

  enddo scatter_members

  ! Deallocate buffers
  if ( mod( tid, proc_interval) == 0) deallocate( dat4d)
  if ( mod( tid, proc_interval) == 0) deallocate( scatbuffer)
  deallocate(recvbuffer)
  call MPI_BARRIER( comm, ierr )


end subroutine read_ensemble_scat
...
! Used if the number of processes is larger than number of members
!==================================================================
subroutine read_ensemble_scat2(i_unit)
  ...
  ! Pass information, 1 variable at a time.
  allocate( scatbuffer( (ni+2)*nicpu * (nj+2)*njcpu * nk ) )
  allocate( recvbuffer((ni+2)*(nj+2)*nk) )
  allocate( loc4d( ni+2, nj+2, nk ) )

  wrf_field_scat_loop: do m = 1, nv
    ...
    prep_slab_scatter: if ( mod( tid, proc_interval ) == 0   &
      ...
      do sid2 = 0, nicpu*njcpu-1
        ...
        ! Determine start index in scatter buffer
        st_ind = sid2 * (ni+2)*(nj+2)*nk+1
        ed_ind = st_ind-1 + (ni+2)*(nj+2)*nk
        ...
        ! Load scattering buffer
        scatbuffer( st_ind:ed_ind ) &
        = reshape( dat4d( ist2:ied2, jst2:jed2, :, m ), (/(ni+2)*(nj+2)*nk/) )
    ...
    scatter_members: do ie = 1, numbers_en
      ...
      if ( flag_recv == 1 ) then
        ...
        call MPI_SCATTER( scatbuffer, (ni+2)*(nj+2)*nk   , MPI_REAL, &
                          recvbuffer, (ni+2)*(nj+2)*nk   , MPI_REAL, &
                          root, r_comm, ierr )
        n = int((ie - gid - 1)/nmcpu) + 1

        ! Interpret receiver buffer
        loc4d = 0.
        loc4d = reshape( recvbuffer, (/ni+2, nj+2, nk/) )

        ! Loading slab
        x(:,:,:,m,n) = loc4d(2:ni+1, 2:nj+1, :)

        ! Loading the boundaries
        if (iid > 0      ) x_we_bdy(1,:,:,m,n) = loc4d(1     , 2:nj+1,:)
        if (iid < nicpu-1) x_we_bdy(2,:,:,m,n) = loc4d(ni+2  , 2:nj+1,:)
        if (jid > 0      ) x_sn_bdy(1,:,:,m,n) = loc4d(2:ni+1, 1     ,:)
        if (jid < njcpu-1) x_sn_bdy(2,:,:,m,n) = loc4d(2:ni+1, nj+2  ,:)

        ! Load the corners
        if ( iid > 0       .and. jid > 0      ) x_crnrs(1,1,:,m,n) = loc4d(1   ,1   ,:)
        if ( iid > 0       .and. jid < njcpu-1) x_crnrs(1,2,:,m,n) = loc4d(1   ,nj+2,:)
        if ( iid < nicpu-1 .and. jid > 0      ) x_crnrs(2,1,:,m,n) = loc4d(ni+2,1   ,:)
        if ( iid < nicpu-1 .and. jid < njcpu-1) x_crnrs(2,2,:,m,n) = loc4d(ni+2,nj+2,:)
...
end subroutine read_ensemble_scat2
...
!========================================================================================
subroutine output(o_unit,ix,jx,kx,kxs,ni,nj,nk,nv,nm,s_comm,sid,iid,jid,xout,times)
use namelist_define
use netcdf
use wrf_tools
use mpi_module
implicit none
integer, intent(in) :: o_unit,ix,jx,kx,kxs,ni,nj,nk,nv,nm,sid,iid,jid,s_comm
...
!===================================================================
! Memory-efficient single-member output procedure
! Written by: M-Y. Chan (Oct 2020)
!==================================================================
subroutine output_RAM_efficient(o_unit,ix,jx,kx,kxs,ni,nj,nk,nv,nm,s_comm,sid,iid,jid,xout,times)
  ...
  integer :: ii,jj,kk,ie,n,m,fid,istart,iend,jstart,jend
  ! Variables relating to passing decomposed fields
  integer :: iid2, jid2, ist2, ied2, jst2, jed2, sid2

  ! Define output file name
  write(filename,'(a5,i5.5)') 'fort.',o_unit

  ! Open file on root sid
  if (sid == 0) call open_file(filename,nf_write,fid)


  ! Iterate over variables
  wrf_field_loop: do m = 1, nv

    ! Figuring out variable dimensions
    call wrf_var_dimension(filename,enkfvar(m),ix,jx,kx,kxs,ii,jj,kk)

    ! Non-root processes send their info to root sid
    mem_sid_send: if ( sid > 0 ) then

      ! Determining member slab bounds
      istart=iid*ni+1
      iend=(iid+1)*ni
      jstart=jid*nj+1
      jend=(jid+1)*nj
      ...
      ! Send information to root proc
      call MPI_Send( xout(1:iend-istart+1,1:jend-jstart+1,1:kk,m), &
                     (iend-istart+1)*(jend-jstart+1)*kk, MPI_REAL, &
                     0, sid, s_comm, ierr )
    endif mem_sid_send

    ! Root sid process receive from non-root sid
    ! and then store to wrf file
    root_sid_proc: if ( sid == 0 ) then

      ! Allocate receiver array
      allocate( dat3d( ii, jj, kk ) )
      dat3d=0.

      ! Load info from sid=0
      istart=iid*ni+1
      iend=(iid+1)*ni
      jstart=jid*nj+1
      jend=(jid+1)*nj
      if(iid==(nicpu-1)) iend=ii
      if(jid==(njcpu-1)) jend=jj
      dat3d( istart:iend, jstart:jend, 1:kk ) = xout(:,:,:,m)

      ! Receive info from sid > 0
      recv_mem_sid_loop: do sid2 = 1, nicpu*njcpu -1
        ...
        jst2=jid2*nj+1
        jed2=(jid2+1)*nj
        if(iid2==(nicpu-1)) ied2=ii
        if(jid2==(njcpu-1)) jed2=jj
        ! Receive
        call MPI_Recv( dat3d( ist2:ied2, jst2:jed2, 1:kk ), &
                       (ied2-ist2+1)*(jed2-jst2+1)*kk, MPI_REAL, &
                       sid2, sid2, s_comm, status, ierr)
      enddo recv_mem_sid_loop

      ! Write to file
      write_to_file: if ( kk > 1 ) then
        call write_variable3d(fid,enkfvar(m),ii,jj,kk,1,dat3d)
      else if (kk == 1 ) then
        call write_variable2d(fid,enkfvar(m),ii,jj,1,dat3d)
      endif write_to_file

      ! Deallocate dat3d as next variable might have different dims
      deallocate(dat3d)
...
end subroutine output_RAM_efficient



!===================================================================
! Ensemble output procedure using MPI_REDUCE
! Written by: M-Y. Chan (Oct 2020)
!==================================================================
! o_unit is either 80010 or 90010
subroutine output_full_ensemble_mpireduce(o_unit,sid,gid,iid,jid)
  ...
  ! Variable static allocations
  implicit none
  integer, intent(in) :: o_unit, sid, gid, iid, jid
  character (len=10) :: filename
  integer :: ii,jj,kk,ie,n,m,fid,istart,iend,jstart,jend, tid,k, w_comm, w_flag
  ! Variables relating to slab boundaries
  integer :: ist, ied, jst, jed
  ! Small buffer to pass information between threads
  real, dimension(ix+1,jx+1,numbers_en+1) :: buffer_recv, buffer_send
  ! Arrays to hold fields reconstituted from buffers
  real, allocatable, dimension(:,:,:) :: dat3d

  ! Shortening proc name
  tid = my_proc_id

  ! Setting up communicator for writer procs
  if ( tid + 1 <= numbers_en+1) then
    w_flag = 1
  else
    w_flag = 0
  endif
  call MPI_COMM_SPLIT( comm, w_flag, tid, w_comm, ierr)

  ! The first numbers_en+1 processes are in charge of writing to file.
  ! Opening relevant output files
  if ( tid +1 <= numbers_en+1 ) then
    ! FYI: for writer processes, tid+1 is the ensemble member id.
    write(filename,'(a5,i5.5)') 'fort.',o_unit + tid + 1
    call  open_file(filename,nf_write,fid)
  endif
  ...
  wrf_field_loop: do m = 1, nv
    ...
    ! Allocate reconstituted array in writer processes
    if ( tid +1 <= numbers_en+1 ) then
      allocate( dat3d( ii,jj,kk) )
      dat3d = 0.
    endif
    ...
    ! Gathering information from all processes to the writing processes,
    ! one model layer at a time
    k_buffer_loop: do k = 1,kk

      ! Initializing buffers
      buffer_recv = 0.; buffer_send = 0.

      ! Fill in buffers
      mem_into_buffer_loop: do n = 1,nm
        ie = (n-1)*nmcpu + gid +1
        if ( ie <= numbers_en+1 ) then
          buffer_send( ist:ied, jst:jed, ie ) &
          = x(1:ied-ist+1, 1:jed-jst+1, k, m, n)
        endif
      enddo mem_into_buffer_loop

      ! Communicate buffer to process 0
      call MPI_REDUCE( buffer_send, buffer_recv, &
                       (ix+1)*(jx+1)*(numbers_en+1), &
                       MPI_REAL, MPI_SUM,0, comm, ierr )

      ! Communicate buffer to all writer processes and reconstitute array
      if ( tid + 1 <= numbers_en+1 ) then
        call MPI_BCAST( buffer_recv, (ix+1)*(jx+1)*(numbers_en+1), &
                        MPI_REAL, 0, w_comm, ierr)
        call MPI_BARRIER( W_COMM, IERR)
        dat3d(:,:,k) = buffer_recv(1:ii,1:jj,tid+1)
      endif
    ...
    ! At this point, writer processes should have reconstituted the array from
    ! the layer-by-layer communication method.

    ! Now proceeding to write the reconstituted array using writer processes
    ! Then, deallocate dat3d from writer procs
    writer_proc_output: if ( tid +1 <= numbers_en+1) then
      ...
      ! Deallocate reconstituted array
      deallocate( dat3d )
    endif writer_proc_output

    call MPI_BARRIER( COMM, IERR )

  enddo wrf_field_loop

  ! Close netcdf file in all writer processes
  if( tid + 1 <= numbers_en ) &
    call close_file(fid)

end subroutine output_full_ensemble_mpireduce



!===================================================================
! Ensemble output procedure using MPI_GATHER
! Written by: M-Y. Chan (Oct 2020)
!==================================================================
! o_unit is either 80010 or 90010
subroutine output_full_ensemble_gather(o_unit)
  ...
  character (len=10) :: filename
  integer :: ii,jj,kk,ie,n,m,fid,istart,iend,jstart,jend, tid,k, w_comm, w_flag
  integer :: root_proc, iid, jid, gid, sid, root, proc_interval
  ! Variables relating to slab boundaries
  integer :: ist, ied, jst, jed, flag_send, st_ind, ed_ind
  integer :: iid2, jid2, sid2, ist2, ied2, jst2, jed2,ie2
  ! Arrays to hold fields reconstituted from buffers
  real, allocatable, dimension(:,:,:) :: dat3d
  real, allocatable, dimension(:,:,:,:) :: dat4d
  real, allocatable, dimension(:) :: recv_buffer
  integer, dimension(numbers_en+1) :: write_procs
  ...
  ! Determining writer processes
  write_procs = -9999
  proc_interval = int( nprocs / (numbers_en+1) )
  do n = 0, numbers_en
    write_procs(n+1) = n * proc_interval
  enddo


  ! All writer processes open WRF files and prep recv_buffer
  ! --------------------------------------------------------
  prep_file: if ( mod( tid, proc_interval ) == 0 &
                 .and. int(tid/proc_interval)<numbers_en+1 ) then

    ! Allocate receiver buffer and interpreter
    allocate( recv_buffer( ni*nicpu * nj*njcpu * nk * nv ) )
    allocate( dat4d( ni*nicpu, nj*njcpu, nk, nv ) )
    recv_buffer = 0.; dat4d = 0.;

    ! Open file
    write( filename,'(a5,i5.5)') 'fort.',o_unit+ int(tid/proc_interval) + 1
    call  open_file(filename,nf_write,fid)

  endif prep_file


  ! Gather all slab info into writer processes
  ! ------------------------------------------
  gather_slabs: do ie = 1, numbers_en+1

    ! Identify write proc to send to
    root_proc = write_procs(ie)

    ! Check if process has info relevant to member
    flag_send = 0
    do n = 1, nm
      ie2 = (n-1)*nmcpu + gid+1
      if (ie2 == ie) then
    ...
    enddo
    if (tid == root_proc) flag_send = 1

    ! Generate communicator for relevant procs
    call MPI_COMM_SPLIT( comm, flag_send, sid, w_comm, ierr )

    ! Gather information.
    if (flag_send == 1) then
      n = int( (ie-gid-1)/nmcpu ) + 1
      if ( tid == root_proc) call MPI_COMM_RANK( w_comm, root, ierr )
      call MPI_BCAST( root, 1, MPI_INTEGER, root_proc, comm, ierr)

      call MPI_GATHER( reshape(x(:,:,:,:,n), (/ni*nj*nk*nv/)), ni*nj*nk*nv, &
                       MPI_REAL, recv_buffer, ni*nj*nk*nv, MPI_REAL, root, w_comm, &
                       ierr)
    endif
    call MPI_BARRIER(comm, ierr)
    call MPI_COMM_FREE( w_comm, ierr )
    call MPI_BARRIER( comm, ierr )

  enddo gather_slabs


  ! Writer processes interpret data and writes to file
  ! --------------------------------------------------
  write_file: if ( mod( tid, proc_interval ) == 0 &
                   .and. int(tid/proc_interval) <numbers_en+1) then

    ! Interpreting buffer
    read_buffer: do sid2 = 0, nicpu*njcpu-1

      ! Slab boundinfg indices
      iid2 = mod( sid2, nicpu )
      jid2 = int( sid2/ nicpu )
      ist2 = iid2*ni+1
      ied2 = (iid2+1) * ni
      ...
      st_ind = ni*nj*nk*nv*sid2 +1
      ed_ind = ni*nj*nk*nv*(sid2+1)

      ! Loading buffer into dat4d
      dat4d(ist2:ied2, jst2:jed2,:,:) &
      = reshape( recv_buffer( st_ind:ed_ind ), (/ni,nj,nk,nv/) )
    ...
    wrf_field_loop: do m = 1, nv
      ...
      ! Load data into 3d arr
      allocate( dat3d( ii,jj,kk) )
      dat3d(:,:,:) = dat4d(1:ii,1:jj,1:kk,m)
      ...
      ! Deallocate and move on to next variable
      deallocate( dat3d )

    enddo wrf_field_loop

    ! close file after writing variables
    call close_file( fid )
...
end subroutine output_full_ensemble_gather

!==============================================================================
subroutine wrf_var_dimension ( wrf_file, var, ix, jx, kx, kxs, ii, jj, kk )
   use netcdf
...
!=======================================================================================
 ...
 FUNCTION beam_conv_simple(numx_crtm, numy_crtm, Tb, obs_ii, obs_jj, dx_wrf, dy_wrf, efov_a, efov_p)

   integer, intent(in) :: numx_crtm, numy_crtm  ! x and y index of observation relative to dimension lenghts of Tb
   real, intent(in), dimension(numx_crtm,numy_crtm) :: Tb  ! simulated brightness temperatures
   real, intent(in) :: obs_ii, obs_jj     ! location of observation converted to indexing of the Tb subdomain of the wrf domain
   real, intent(in) :: dx_wrf, dy_wrf     ! grid spacing of wrf (and CRTM) in km
   real, intent(in) :: efov_a, efov_p        ! effective field of view (based on half-power beam width) along- and cross-scan dimensions in km

   real :: beam_conv_simple

   real, parameter :: efov_multiplier = sqrt(2.)
   real, parameter :: pi = 4*ATAN(1.)

   real :: Tb_conv, weight_sum, weight, distance
   integer xx, yy
   integer, dimension(numx_crtm) :: all_x
   integer, dimension(numy_crtm) :: all_y
   real, dimension(numx_crtm) :: distance_x
   real, dimension(numy_crtm) :: distance_y


   max_distance = efov_multiplier * sqrt(efov_a*efov_p/dx_wrf/dy_wrf/pi)
   ...
   distance_x = all_x - obs_ii
   distance_y = all_y - obs_jj

   Tb_conv = 0.
   weight_sum = 0.
   do xx = 1,numx_crtm
   do yy = 1,numy_crtm
   distance = sqrt(distance_x(xx)**2 + distance_y(yy)**2)
   weight = max(0.d0, max_distance - distance)
   Tb_conv = Tb_conv + Tb(xx,yy) * weight
   if (Tb(xx,yy) .NE. Tb(xx,yy)) then
     write(*,*) '  at x=',xx,' y=',yy,' Tb is ', Tb(xx,yy)
     write(*,*) '    beam_conv_simple is now =',(Tb_conv / weight_sum)
   endif
   weight_sum = weight_sum + weight
   enddo
   enddo

   if (weight_sum < 0.5) then
     WRITE(*,*) '  in beam_conv_simple for x=', obs_ii, ', y=', obs_jj
     write(*,*) '  weight_sum: ', weight_sum
   endif
   beam_conv_simple = (Tb_conv / weight_sum)

 END FUNCTION beam_conv_simple

!=======================================================================================

 FUNCTION beam_conv_gaussian_simple(numx_crtm, numy_crtm, Tb, obs_ii, obs_jj, dx_wrf, dy_wrf, efov_a, efov_p)
   integer, intent(in) :: numx_crtm, numy_crtm  ! x and y index of observation relative to dimension lenghts of Tb
   real, intent(in), dimension(numx_crtm,numy_crtm) :: Tb  ! simulated brightness temperatures
   real, intent(in) :: obs_ii, obs_jj     ! location of observation converted to indexing of the Tb subdomain of the wrf domain
   real, intent(in) :: dx_wrf, dy_wrf     ! grid spacing of wrf (and CRTM) in km
   ...
   real :: beam_conv_gaussian_simple

   real, parameter :: pi = 4*ATAN(1.)

   real :: sigma, max_distance, distance, weight, Tb_conv, weight_sum
   integer i,j,xx,yy
   ...
   ! in unit gridpoints, assuming dx_wrf == dy_wrf
   sigma = 0.5 * (((efov_a+efov_p)/2) / 1.18) / ((dx_wrf+dy_wrf)/2)
   max_distance = 2 * sigma

   min_x = max(1,        floor(obs_ii) - ceiling(max_distance))
   max_x = min(numx_crtm,ceiling(obs_ii) + ceiling(max_distance))
   min_y = max(1,        floor(obs_jj) - ceiling(max_distance))
   max_y = min(numy_crtm,ceiling(obs_jj) + ceiling(max_distance))

   all_x = (/(xx, xx=min_x,max_x,1)/)
   all_y = (/(yy, yy=min_y,max_y,1)/)
   ...
   weight_sum = 0.
   do i = 1,size(all_x)
   xx = min(numx_crtm,max(1,all_x(i)))
   do j = 1,size(all_y)
   yy = min(numy_crtm,max(1,all_y(j)))
   distance = sqrt(distance_x(i)**2 + distance_y(j)**2)
   ...
   if ((Tb_conv / weight_sum) .NE. (Tb_conv / weight_sum)) then
     WRITE(*,*) '  this is an NaN beam convolution'
   endif
   beam_conv_gaussian_simple = (Tb_conv / weight_sum)

 END FUNCTION beam_conv_gaussian_simple
