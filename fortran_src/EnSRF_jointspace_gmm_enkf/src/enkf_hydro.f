...
!
!--------------------------------------------------------------------------------------------------------------
subroutine enkf (wrf_file,ix,jx,kx,kxs,ni,nj,nk,nv,nm,g_comm,s_comm,gid,sid,iid,jid,xm,x,ind,proj,times)
use constants
use namelist_define
use obs_define
use mpi_module
...
do iob=1,obs%num
   ...
   if ( use_simulated ) then
      ...
      if ( obstype(1:5) == 'Radar' ) then
         ...
         if ( obstype == 'longitude ' ) obs%dat(iob) = center_xb(1)
   ...
   do n = 1, nm
     ...
     if (ie<=numbers_en+1) then
       ...
       if ( obstype(1:5) == 'Radar' ) then
         ...
         call hurricane_center_assimilation(filename,ix,jx,kx,int(obs%position(iob,1)),int(obs%position(iob,2)), &
                                            center_xb,znu,znw,xlong,xlat,proj)
         if ( obstype == 'longitude ' ) ya(iob,ie) = center_xb(1)
