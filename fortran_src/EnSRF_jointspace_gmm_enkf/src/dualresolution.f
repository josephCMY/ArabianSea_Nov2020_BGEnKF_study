...
!----------------------------------------------------------------
  ...
  integer            :: ixl, jxl, kxl, kxls, ixh, jxh, kxh, kxhs
...
!  call get_var_ij ( xfl_file, 'T         ', ixl, jxl, kxl )
   call get_ij ( xfl_file, ixl, jxl, kxl, kxls )

  write(xfh_file,'(a5,i5.5)')'fort.',xfh_unit
!  call get_var_ij ( xfh_file, 'T         ', ixh, jxh, kxh )
   call get_ij ( xfh_file, ixh, jxh, kxh, kxhs )
...
! variables loop
  ...
  do n = 1, num_var
     ...
     call wrf_var_dimension ( xal_file, enkfvar(n), ixl, jxl, kxl, kxls, iil, jjl, kkl )
     call wrf_var_dimension ( xal_file, enkfvar(n), ixh, jxh, kxh, kxhs, iih, jjh, kkh )
